  ...
  return (
    <Shell page="hub">
      <PageHeader
        ...
        subtitle={
          <>
            Terça, 13 mai 2026 · <span className="font-mono">07:42 BRT</span> · 12 corridas
            pendentes nas próximas 4h
          </>
        }
        actions={
          <>
      ...
      <div className="px-8 py-5 space-y-5 overflow-auto h-[calc(100%-128px)]">
        ...
        <div className="grid grid-cols-4 gap-4">
          <KPI
            ...
            sub="↑ 8 vs ontem"
            accent="info"
            spark={[0.3, 0.5, 0.4, 0.6, 0.5, 0.7, 0.8]}
          />
          <KPI
            ...
            sub="Westgard · 28d"
            accent="success"
            spark={[0.7, 0.8, 0.75, 0.85, 0.9, 0.86, 0.92]}
          />
          <KPI
            label="NCs abertas"
            value="7"
            sub="3 vencidas · 2 críticas"
            accent="danger"
            spark={[0.5, 0.6, 0.4, 0.7, 0.8, 0.5, 0.6]}
          />
          <KPI
            label="CAPAs em prazo"
            value="89%"
            sub="22 ativas · 4 a vencer"
            accent="warning"
            spark={[0.4, 0.6, 0.5, 0.7, 0.6, 0.55, 0.5]}
          />
        </div>
            ...
            <table className="w-full text-[12.5px]">
              <thead>
                <tr style={{ color: 'var(--text-faint)' }}>
                  <th className="text-left text-[10px] uppercase tracking-wider font-semibold pb-2.5 pl-1">
                    Setor
                  </th>
                  <th className="text-left text-[10px] uppercase tracking-wider font-semibold pb-2.5">
                    Analito · Lote
                  </th>
                  <th className="text-left text-[10px] uppercase tracking-wider font-semibold pb-2.5">
                    Nível
                  ...
                  <th className="text-right text-[10px] uppercase tracking-wider font-semibold pb-2.5 pr-1">
                    Iniciado
                  </th>
                </tr>
              </thead>
              <tbody>
                {OPS_ROWS.map((r, i) => (
                  <tr key={i} style={{ borderTop: '1px solid var(--border-hairline)' }}>
                    <td className="py-2.5 pl-1">
                      <span className="inline-flex items-center gap-2">
                        <span
                          ...
                          style={{ background: r.sectorColor }}
                        />
                    ...
                    >
                      {r.analyte} <span style={{ color: 'var(--text-faint)' }}>· {r.lot}</span>
                    </td>
                    <td className="py-2.5">
                      <span
                    ...
                    <td
                      className="py-2.5 text-right pr-1 font-mono text-[11px]"
                      style={{ color: 'var(--text-muted)' }}
                    >
                      {r.time}
                    </td>
          ...
          <Card title="Alertas" padded={false}>
            <ul>
              {ALERTS.map((a, i) => (
                ...
                >
                  <span
                    className="mt-0.5 w-7 h-7 rounded-md inline-flex items-center justify-center shrink-0"
        ...
        <div className="grid grid-cols-3 gap-4">
          <Card
            title="Westgard · últimos 7 dias"
            actions={
              <button className="text-[11px] font-medium" style={{ color: 'var(--accent-600)' }}>
                Detalhes →
              </button>
            }
          >
            ...
            <div className="space-y-3">
              {COMPL.map((c, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="text-[11px] w-24 shrink-0" style={{ color: 'var(--text-muted)' }}>
                    {c.label}
                  </div>
                  <div
                  ...
                  <div
                    className="text-[11px] w-9 text-right font-mono tabular-nums"
                    style={{ color: 'var(--text-strong)' }}
                  >
                    {c.pct}%
                  </div>
                ...
                <li key={i} className="flex items-center">
                  ...
                  <div className="text-right">
                    <div
                      ...
                      style={{ color: c.urgent ? 'var(--danger-500)' : 'var(--text-body)' }}
                    >
                      {c.due}
                    </div>
...
const ALERTS = [
  {
    tone: 'danger',
    title: 'Westgard 1-3s — Creatinina nível 2',
    body: 'Corrida rejeitada. Bloqueio automático de liberação até reanálise.',
    time: 'há 12 min',
    module: 'CQI / Bioquímica',
  },
  {
    tone: 'warning',
    title: 'Temperatura geladeira reagentes',
    body: 'Pico de 9.4 °C às 04:18. Faixa: 2–8 °C. Trend retornou em 12 min.',
    time: 'há 3h',
    module: 'Temperatura',
  },
  {
    ...
    body: 'Última: 28 abr. Periodicidade 14 dias. Bloqueio em 2 dias.',
    time: 'hoje',
    module: 'Equipamentos',
  },
  {
    tone: 'danger',
    title: 'NC-2026-041 sem CAPA atribuída',
    body: 'Não conformidade crítica aberta há 5 dias.',
    time: 'há 5d',
    module: 'NC',
  },
];
      ...
      {bars.map((b, i) => {
        ...
        return (
          <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
            <div
              className="w-full flex flex-col-reverse items-stretch h-24 rounded overflow-hidden"
              style={{ background: 'var(--surface-muted)' }}
            >
            ...
            <div
              ...
              style={{ color: b.rj > 0 ? 'var(--danger-500)' : 'var(--text-muted)' }}
            >
              {tot}
            </div>
          </div>
  ...
  return (
    <Shell page="analyzer">
      <PageHeader
        ...
        subtitle={
          <>
            Sysmex XN-1000 · Hematologia · 28 dias · <span className="font-mono">N=84 medidas</span>
          </>
        }
        actions={
          <>
        ...
        <div className="space-y-4 min-w-0">
              ...
              <div>
                ...
                <div className="mt-1 flex items-center gap-3 font-mono text-[12px]">
                  <span style={{ color: 'var(--text-body)' }}>
                    Média <strong style={{ color: 'var(--text-strong)' }}>13.62</strong> g/dL
                  </span>
          ...
          >
            <table className="w-full text-[12.5px]">
              <thead>
                <tr style={{ color: 'var(--text-faint)' }}>
                  <th className="text-left text-[10px] uppercase tracking-wider font-semibold py-2.5 pl-5">
                    Data
                  </th>
                  <th className="text-left text-[10px] uppercase tracking-wider font-semibold py-2.5">
                    Operador
                  </th>
                  <th className="text-right text-[10px] uppercase tracking-wider font-semibold py-2.5">
                    Valor
                  </th>
                  <th className="text-right text-[10px] uppercase tracking-wider font-semibold py-2.5">
                    z-score
                  </th>
                  <th className="text-left text-[10px] uppercase tracking-wider font-semibold py-2.5 pl-3">
                    Regras
                  </th>
                  <th className="text-left text-[10px] uppercase tracking-wider font-semibold py-2.5 pr-5">
                    Status
                  </th>
                </tr>
              </thead>
              <tbody>
                {HX.map((r, i) => (
                  <tr key={i} style={{ borderTop: '1px solid var(--border-hairline)' }}>
                    <td
                      className="py-2.5 pl-5 font-mono text-[11.5px]"
                      style={{ color: 'var(--text-body)' }}
                    >
                      {r.dt}
                    </td>
                    <td className="py-2.5" style={{ color: 'var(--text-body)' }}>
                      {r.op}
                    </td>
                    <td
                    ...
                    <td className="py-2.5 pl-3">
                      <div className="flex gap-1">
                        {r.rules.map((rr) => (
                          <WgChip key={rr} rule={rr} active={r.rejected || rr === '1-2s'} />
                        ))}
                      </div>
                    </td>
                    <td className="py-2.5 pr-5">
                      <Badge tone={r.rejected ? 'danger' : r.rules.length ? 'warning' : 'success'}>
                        {r.rejected ? 'Rejeitada' : r.rules.length ? 'Aviso' : 'Aprovada'}
                      </Badge>
                    </td>
                  </tr>
                ))}
          ...
          <Card title="Estado do lote">
            <dl className="space-y-2 text-[12px]">
              <div className="flex justify-between">
                <dt style={{ color: 'var(--text-muted)' }}>Fabricante</dt>
                <dd style={{ color: 'var(--text-strong)' }}>Sysmex e-Check</dd>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <dt style={{ color: 'var(--text-muted)' }}>Aberto em</dt>
                <dd className="font-mono" style={{ color: 'var(--text-strong)' }}>
                  02 abr 2026
                </dd>
              </div>
              <div className="flex justify-between">
                <dt style={{ color: 'var(--text-muted)' }}>Estabilidade</dt>
                <dd className="font-mono" style={{ color: 'var(--text-strong)' }}>
                  34 d restantes
                </dd>
              </div>
            ...
            <div className="mt-3.5 pt-3" style={{ borderTop: '1px solid var(--border-hairline)' }}>
              ...
              >
                <div
                  className="h-full rounded-full"
                  style={{ width: '28%', background: 'var(--accent-600)' }}
                />
              </div>
            </div>
          </Card>
              ...
              {WG_RULES.map((r, i) => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{ color: r.type === 'rj' ? 'var(--danger-500)' : 'var(--warning-500)' }}
                  >
                    {r.type === 'rj' ? 'reject' : 'warn'}
                  </span>
                </li>
              ))}
            </ul>
          ...
          <Card title="Vínculos">
            <ul className="space-y-2.5 text-[12px]">
              <li className="flex items-center gap-2">
                <I.File size={13} />
                <a style={{ color: 'var(--accent-600)' }}>NC-2026-038 · Aviso recorrente</a>
              </li>
              <li className="flex items-center gap-2">
                <I.Camera size={13} />
                <a style={{ color: 'var(--accent-600)' }}>Evidência fotográfica (2)</a>
              </li>
              <li className="flex items-center gap-2">
                <I.Books size={13} />
                <a style={{ color: 'var(--accent-600)' }}>Procedimento POP-CQI-014</a>
              </li>
            </ul>
          </Card>
        </aside>
...
const HX = [
  ...
  { dt: '07 mai 06:47', op: 'R. Coutinho', val: '13.62', z: 0.0, rules: [], rejected: false },
...
function LJChart() {
  ...
  const pts = HX.slice()
    .reverse()
    .concat([{ val: '13.84', z: 0.79 }])
    .slice(0, 14)
    .map((p, i) => ({ x: i + 1, z: Number(p.z || (Math.random() * 2 - 1).toFixed(2)) }));
  // Build extra synthetic pts
  while (pts.length < 28)
    pts.push({
      x: pts.length + 1,
      z: (Math.random() * 3 - 1.5) * (Math.random() > 0.85 ? 2.2 : 1),
    });
      ...
      <div className="px-8 py-6 max-w-[1100px] mx-auto space-y-6 overflow-auto h-[calc(100%-100px)]">
        ...
        <Card title="2 · Inserir valores · HC-2904">
          <div className="grid grid-cols-3 gap-5">
            <div className="col-span-2">
              <table className="w-full text-[12.5px]">
                <thead>
                  <tr style={{ color: 'var(--text-faint)' }}>
                    <th className="text-left text-[10px] uppercase tracking-wider font-semibold pb-2">
                      Analito
                    </th>
                    <th className="text-left text-[10px] uppercase tracking-wider font-semibold pb-2">
                      Unidade
                    </th>
                    <th className="text-right text-[10px] uppercase tracking-wider font-semibold pb-2">
                      Alvo
                    </th>
                    <th className="text-right text-[10px] uppercase tracking-wider font-semibold pb-2">
                      Nv 1
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ borderTop: '1px solid var(--border-hairline)' }}>
                      <td className="py-2.5 font-medium" style={{ color: 'var(--text-strong)' }}>
                        {r.an}
                      </td>
                      <td
                        className="py-2.5 font-mono text-[11px]"
                        style={{ color: 'var(--text-muted)' }}
                      >
                        {r.un}
                      </td>
                      ...
                      <td className="py-2.5 text-right">
                        <input
                          defaultValue={r.n1}
                          className="w-20 px-2 h-7 rounded font-mono text-[12px] tabular-nums text-right"
                          style={{
                      ...
                      <td className="py-2.5 text-right">
                        <input
                          ...
                          className="w-20 px-2 h-7 rounded font-mono text-[12px] tabular-nums text-right"
                          style={{
                            background: r.reject ? 'var(--danger-50)' : 'var(--surface-muted)',
                      ...
                      <td className="py-2.5 text-right">
                        <div className="flex justify-end gap-1">
                          {r.wg.length ? (
                            r.wg.map((g) => <WgChip key={g} rule={g} active />)
                          ) : (
            ...
            <aside className="space-y-3">
              ...
              <textarea
                placeholder="Causa raiz (obrigatório se rejeitada)"
                rows={4}
          ...
          >
            ...
            <Btn kind="primary" size="md" className="ml-auto" icon={<I.ChevR size={13} />}>
              Revisar & aprovar
            </Btn>
          </div>
        </Card>
      ...
      {steps.map((s, i) => (
        <React.Fragment key={i}>
          <div className="flex items-center gap-2.5">
            <div
              className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-semibold font-mono"
              style={{
                background: s.done
                  ? 'var(--success-500)'
                  : s.active
            ...
            <span
              ...
              style={{
                color: s.active || s.done ? 'var(--text-strong)' : 'var(--text-faint)',
          ...
          {i < steps.length - 1 && (
            <div
              ...
              style={{ background: s.done ? 'var(--success-500)' : 'var(--border-soft)' }}
            />
          )}
        </React.Fragment>
      ))}
