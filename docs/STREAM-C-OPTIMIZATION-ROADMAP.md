...
onSnapshot(collection(firestore, 'educacaoContinuada', labId, 'execucoes'), (snap) =>
...
export async function traceQuery(name: string, queryFn: () => Promise<any>) {
...
// Usage in hooks:
const pops = await traceQuery('pops_list_load', () => queryPOPs(labId));
```
