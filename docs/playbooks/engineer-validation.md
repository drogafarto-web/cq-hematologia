# Engineer — Validação de Código

> Modo invocado quando o CTO pedir "revisa", "audit", "valida engineer". Aplica-se em qualquer ferramenta (Cursor, Codex CLI, Claude Code, agente humano).
...
- **Zero endpoints sem validação de input** — toda boundary valida dados (Zod nas callables)
- **Zero `console.log` em produção** — logging estruturado com níveis corretos
...
- **HC Quality específico:** chain-hash de `/insumo-movimentacoes` é sagrado — nunca delete, nunca reset.

### Infraestrutura
- Docker (se aplicável): rebuild necessário após mudanças de código (não só restart)
