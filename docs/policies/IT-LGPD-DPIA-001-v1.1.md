...
> **Nota operacional:** este arquivo é o *patch v1.1* em `docs/policies/`. A publicação no SGQ via fluxo `revisao-emitida` (RN-SGQ-03) está pendente de login do RT no módulo SGQ em produção. Quando publicado, o doc `/labs/{labId}/sgq-documentos/{id}` com `codigo='IT-LGPD-DPIA-001'` v1.0 transiciona para `status='obsoleto'`, e este v1.1 entra como `status='vigente'` com `substitui` apontando ao v1.0 e `substituidoPor` ainda nulo. Cross-link ADR-0016 declarado na Section 4 (Riscos Potenciais).
