...
const ready = await db.collectionGroup('notivisa-outbox').where('status', '==', 'PENDING')
  .where('nextRetry', '<=', now).get();
...
export const manuallyRetryNotivisaCallable = onCall(async (data: { queueDocId, labId }) => {
  ...
  await auditLog('notivisa_manual_retry', { queueDocId: data.queueDocId, adminUid: request.auth.uid });
});
```
