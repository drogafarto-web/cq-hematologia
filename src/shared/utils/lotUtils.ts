...
export function groupByMonth(lots: ControlLot[]): Array<{ key: string; label: string; lots: ControlLot[] }> {
  ...
  for (const lot of sorted) {
    ...
    if (!map.has(key)) {
      const label = new Date(yr, mo, 1).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
