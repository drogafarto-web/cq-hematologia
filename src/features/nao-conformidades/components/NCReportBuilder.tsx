...
export function NCReportBuilder({ labId }: NCReportBuilderProps) {
  ...
  const generateReport = useCallback(async () => {
    ...
    try {
      ...
      const [ncSnap, capaSnap] = await Promise.all([
        getDocs(
          query(collection(db, 'labs', labId, 'naoConformidades'), where('deletadoEm', '==', null)),
        ),
        getDocs(query(collection(db, 'labs', labId, 'capa'), where('deletadoEm', '==', null))),
      ]);
  ...
  const INPUT_CLS =
    'px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white focus:ring-2 focus:ring-violet-500 focus:border-transparent';
      ...
      <div className="print:hidden space-y-4">
        <div>
          ...
          <p className="text-xs text-white/40 mt-1">
            Selecione o período e gere o relatório para impressão/PDF
          </p>
        </div>
        ...
        <div className="space-y-6 print:text-black print:bg-white" id="nc-report">
          ...
          <div className="border-b border-white/10 print:border-black/20 pb-4">
            ...
            <p className="text-xs text-white/50 print:text-gray-600 mt-1">
              Período: {getDateRange()[0].toLocaleDateString('pt-BR')} a{' '}
              {getDateRange()[1].toLocaleDateString('pt-BR')}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              {
                label: 'Abertas',
                value: reportData.abertas,
                color: 'text-red-400 print:text-red-600',
              },
              {
                label: 'Fechadas',
                value: reportData.fechadas,
                color: 'text-emerald-400 print:text-green-600',
              },
              {
                label: 'Vencidas',
                value: reportData.vencidas,
                color: 'text-amber-400 print:text-orange-600',
              },
            ].map((card) => (
              <div
                key={card.label}
                className="p-3 rounded-lg bg-white/[0.03] print:bg-gray-50 border border-white/[0.06] print:border-gray-200"
              >
                <p className="text-[10px] text-white/40 print:text-gray-500 uppercase">
                  {card.label}
                </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-lg bg-white/[0.02] print:bg-gray-50 border border-white/[0.06] print:border-gray-200">
              <p className="text-xs font-semibold text-white/70 print:text-gray-700 mb-2">
                Por Setor
              </p>
              {Object.entries(reportData.porSetor)
                .sort((a, b) => b[1] - a[1])
                .map(([setor, count]) => (
                  <div
                    key={setor}
                    className="flex justify-between text-xs py-1 border-b border-white/[0.04] print:border-gray-100 last:border-0"
                  >
                    <span className="text-white/60 print:text-gray-600">{setor}</span>
                    <span className="text-white/90 print:text-black font-medium tabular-nums">
                      {count}
                    </span>
                  </div>
                ))}
            ...
            <div className="p-4 rounded-lg bg-white/[0.02] print:bg-gray-50 border border-white/[0.06] print:border-gray-200">
              ...
              {Object.entries(reportData.porOrigem)
                .sort((a, b) => b[1] - a[1])
                .map(([origem, count]) => (
                  <div
                    key={origem}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-white/10 print:border-gray-300">
                  <th className="text-left py-2 text-white/50 print:text-gray-500 font-medium">
                    Data
                  </th>
                  <th className="text-left py-2 text-white/50 print:text-gray-500 font-medium">
                    Título
          ...
          <div className="pt-4 border-t border-white/10 print:border-gray-200 text-[10px] text-white/30 print:text-gray-400">
            <p>
              Relatório gerado automaticamente pelo HC Quality — RDC 978/2025 Art. 134 | DICQ 4.14.7
            </p>
          </div>
        </div>
      )}
