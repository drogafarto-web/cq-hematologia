...
function LevelPills({ lots, activeLot, onSelect }: { lots: ControlLot[], activeLot: ControlLot | null, onSelect: (id: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-1.5 ml-4 px-3 py-1 bg-slate-100 dark:bg-white/[0.05] rounded-full border border-slate-200 dark:border-white/[0.08]">
      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 mr-1">Nível:</span>
      <div className="flex gap-1">
        {sorted.map((lot) => {
          const isActive = lot.id === activeLot?.id;
          return (
            <button key={lot.id} type="button" onClick={() => onSelect(lot.id)}
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {ALL_RULES.map((rule) => {
        ...
        return (
          <span key={rule} className={`font-mono text-[11px] px-1.5 py-0.5 rounded border transition-colors ${
            active
...
export function AnaliseScreen({
  lots, activeLot, selectLot,
  validAnalyteId, setSelectedAnalyte,
  statsMode, chartData, currentAnalyte,
  goTo,
}: Props) {
    ...
    <>
      ...
      <div className="flex items-center gap-4 mb-8">
        <div className="flex flex-col">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Análise estatística</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Visualizando tendências e regras de Westgard{activeLot ? ` para ${activeLot.equipmentName}` : ''}
          </p>
        </div>
        ...
        <div className="ml-auto flex gap-2">
          <button type="button" onClick={() => setCurrentView('reports')} className="inline-flex items-center gap-2 h-9 px-3.5 rounded-lg text-sm font-medium bg-white dark:bg-white/[0.05] border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/[0.08] transition-colors">
            <CalendarIcon /> Comparar lotes
          </button>
          <button type="button" onClick={() => setCurrentView('reports')} className="inline-flex items-center gap-2 h-9 px-3.5 rounded-lg text-sm font-medium bg-white dark:bg-white/[0.05] border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/[0.08] transition-colors">
            <DownloadIcon /> Exportar PDF
          </button>
          <button type="button" onClick={() => goTo('nova')} className="inline-flex items-center gap-2 h-9 px-3.5 rounded-lg text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 transition-colors">
      ...
      {!activeLot ? (
        <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] rounded-xl p-10 text-center shadow-sm">
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Nenhum lote de controle ativo</p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 mb-4">Selecione um lote para visualizar o gráfico de controle</p>
          <button type="button" onClick={() => goTo('lotes')}
        ...
        <div className="space-y-4">
          ...
          {activeLot.requiredAnalytes.length > 0 && (
            <AnalyteSelector lot={activeLot} selectedAnalyteId={validAnalyteId} onSelect={setSelectedAnalyte} />
          )}
          ...
          {validAnalyteId && currentAnalyte ? (
            <>
              ...
              <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] rounded-xl overflow-hidden shadow-sm dark:shadow-none">
                <div className="flex items-center px-5 py-3.5 border-b border-slate-100 dark:border-white/[0.06]">
                  <span className="text-sm font-semibold text-slate-700 dark:text-white/80">Regras de Westgard</span>
            ...
            <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] rounded-xl p-8 text-center shadow-sm">
              <p className="text-sm text-slate-400 dark:text-slate-500">Selecione um analito acima para ver o gráfico de Levey-Jennings</p>
            </div>
          )}
        </div>
      )}
