...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium border whitespace-nowrap ${cfg[status] ?? cfg['Pendente']}`}
    >
...
export function HistoricoScreen({ lots, goTo }: Props) {
    ...
    return allRuns.filter((r) => {
      ...
      if (query) {
        ...
        const names = r.results
          .map((res) => ANALYTE_MAP[res.analyteId]?.name ?? res.analyteId)
          .join(' ');
  ...
  const INPUT_CLS =
    'h-8 px-3 text-xs bg-slate-50 dark:bg-white/[0.05] border border-slate-200 dark:border-white/[0.08] rounded-lg text-slate-700 dark:text-slate-300 outline-none focus:border-blue-400 dark:focus:border-blue-500/50 transition-all';
      ...
      <div className="flex items-end gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5">
            Registro imutável · trilha de auditoria completa
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-white/[0.03] border border-blue-200 dark:border-blue-500/20 rounded-xl px-5 py-4 mb-4 shadow-sm dark:shadow-none">
          <div className="flex flex-wrap items-end gap-5">
            ...
            <span className="ml-auto text-xs text-slate-400 dark:text-slate-500 self-end pb-0.5">
              {filtered.length} corrida{filtered.length !== 1 ? 's' : ''} encontrada
              {filtered.length !== 1 ? 's' : ''}
            </span>
          </div>
        </div>
        ...
        {allRuns.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            <p className="text-xs text-slate-300 dark:text-slate-600 mt-1">
              As corridas aparecerão aqui após serem registradas
            </p>
            <button
              type="button"
              onClick={() => goTo('nova')}
                    ...
                    {list.map((r) => {
                      ...
                      const analyteNames =
                        r.results
                          .slice(0, 3)
                          .map((res) => ANALYTE_MAP[res.analyteId]?.name ?? res.analyteId)
                          .join(', ') + (r.results.length > 3 ? ` +${r.results.length - 3}` : '');
                      return (
                        <tr
                          key={r.id}
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-slate-700 dark:text-slate-300 text-xs truncate max-w-[120px]">
                                {r.lotName}
                              </span>
                            </div>
                            ...
                            {violations.length > 0 ? (
                              <div className="flex gap-1 flex-wrap">
                                {violations.slice(0, 3).map((v) => (
                                  <span
                                    key={v}
                                    className="font-mono text-[10px] px-1.5 py-0.5 rounded border bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 border-red-200 dark:border-red-500/20"
                                  >
                                    {v}
                                  </span>
                                ))}
