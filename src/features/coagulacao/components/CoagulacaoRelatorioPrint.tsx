...
export function CoagulacaoRelatorioPrint({
  lot,
  runs,
  lotStatus,
  onClose,
}: CoagulacaoRelatorioPrintProps) {
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-3 bg-slate-100 dark:bg-[#0F1318] border-b border-slate-200 dark:border-white/[0.08] print:hidden">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-slate-900 dark:text-white">
            Relatório de Controle Interno — Coagulação
          </p>
          ...
          <article className="p-10 print:p-0 text-[11px] leading-relaxed">
            ...
            <section className="mb-6">
              ...
              <table className="w-full border-collapse text-[9.5px]">
                ...
                <tbody>
                  {sortedRuns.map((r, idx) => {
                    ...
                    const apOut =
                      r.resultados.atividadeProtrombinica < cfg.ap.low ||
                      r.resultados.atividadeProtrombinica > cfg.ap.high;
                    const rniOut =
                      r.resultados.rni < cfg.rni.low || r.resultados.rni > cfg.rni.high;
                    const ttpaOut =
                      r.resultados.ttpa < cfg.ttpa.low || r.resultados.ttpa > cfg.ttpa.high;
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          className={`px-2 py-1 text-right tabular-nums ${apOut ? 'text-red-600 font-semibold' : ''}`}
                        >
                          {r.resultados.atividadeProtrombinica}
                        </td>
                        <td
                          className={`px-2 py-1 text-right tabular-nums ${rniOut ? 'text-red-600 font-semibold' : ''}`}
                        >
                          {r.resultados.rni.toFixed(2)}
                        </td>
                        <td
                          className={`px-2 py-1 text-right tabular-nums ${ttpaOut ? 'text-red-600 font-semibold' : ''}`}
                        >
                          {r.resultados.ttpa.toFixed(1)}
                        </td>
                        <td
                          className={`px-2 py-1 font-semibold ${isNC ? 'text-red-700' : 'text-emerald-700'}`}
                        >
                          {isNC ? 'R' : 'A'}
                        </td>
              ...
              <p className="text-[8.5px] text-slate-500 mt-1.5">
                Linhas em vermelho indicam não conformidade. Valores individuais em vermelho
                extrapolam o intervalo do fabricante (low–high).
              </p>
            </section>
            ...
            <footer className="mt-10 pt-4 border-t border-slate-300 text-[8.5px] text-slate-500 leading-relaxed">
              <p>
                Documento emitido eletronicamente em conformidade com a{' '}
                <strong>RDC 302/2005</strong>,<strong> RDC 978/2025</strong> (Art. 128 — ação
                corretiva em toda não conformidade), e <strong>RDC 67/2009 · 551/2021</strong>{' '}
                (Tecnovigilância NOTIVISA). Referências técnicas: CLSI H21-A5, H47-A2, H54-A; ISTH
                guidelines (INR/ISI/MNPT). Cada corrida possui assinatura lógica SHA-256 vinculando
                operador, lote e resultados — verificável via QR Code no sistema.
              </p>
