...
import type { AdminLabRecord } from './services/superAdminService';
import {
  fetchAllLabs,
  deleteLabAsAdmin,
} from './services/superAdminService';
import { createLab } from './services/labAdminService';
...
export function LabManagementTab() {
  ...
  async function handleDelete(lab: AdminLabRecord) {
    const confirmed = window.confirm(
      `Excluir "${lab.name}"?\n\nTodos os membros perderão acesso. Esta ação não pode ser desfeita.`
    );
    if (!confirmed) return;
            ...
            >
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-white/50 hover:text-white/80 hover:bg-white/[0.07] transition-all"
                >
                  <UsersIcon />
                  Membros
                </button>
                <button
                  ...
                  className="p-2 rounded-lg text-white/25 hover:text-red-400 hover:bg-red-500/10 disabled:opacity-40 transition-all"
                >
                  <TrashIcon />
                </button>
              </div>
