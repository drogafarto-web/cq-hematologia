...
function AnalyteStatsRow({ analyteId, lot }: { analyteId: string; lot: ControlLot }) {
  ...
  return (
    <tr className="border-t border-gray-200 print:border-gray-300">
      <td className="py-1.5 pl-3 font-mono text-xs font-semibold text-gray-700 print:text-black">{analyte.name}</td>
      <td className="py-1.5 text-center text-xs text-gray-500 print:text-gray-600">{analyte.unit}</td>
      <td className="py-1.5 text-center font-mono text-xs text-gray-700 print:text-black">{mfr ? mfr.mean.toFixed(d) : '—'}</td>
      <td className="py-1.5 text-center font-mono text-xs text-gray-700 print:text-black">{mfr ? mfr.sd.toFixed(d) : '—'}</td>
      <td className="py-1.5 text-center font-mono text-xs text-gray-600 print:text-gray-700">{mfr && mfr.mean > 0 ? `${((mfr.sd / mfr.mean) * 100).toFixed(1)}%` : '—'}</td>
      <td className="py-1.5 text-center font-mono text-xs text-gray-700 print:text-black border-l border-gray-100 print:border-gray-200">{int ? int.mean.toFixed(d) : '—'}</td>
      <td className="py-1.5 text-center font-mono text-xs text-gray-700 print:text-black">{int ? int.sd.toFixed(d) : '—'}</td>
      <td className="py-1.5 text-center font-mono text-xs text-gray-600 print:text-gray-700">{int ? `${int.cv.toFixed(1)}%` : '—'}</td>
      <td className="py-1.5 pr-3 text-center font-mono text-xs text-gray-600 print:text-gray-700">{int ? int.n : '—'}</td>
    </tr>
  );
}
      ...
      <ResponsiveContainer width="100%" height={110}>
        <LineChart data={data} margin={{ top: 8, right: 20, left: 0, bottom: 4 }}>
          ...
          <YAxis domain={[yMin, yMax]} tick={{ fontSize: 8, fill: '#9ca3af' }} tickLine={false}
            axisLine={false} width={36} tickFormatter={(v) => v.toFixed(analyte.decimals)} />
          ...
          <ReferenceLine y={mean - 3*sd} stroke="#ef4444" strokeWidth={0.8} strokeDasharray="3 2" />
          <Line type="monotone" dataKey="value" stroke="#4f46e5" strokeWidth={1.5}
    ...
    <table className="w-full text-xs border-collapse">
      <thead>
        <tr className="bg-gray-100 print:bg-gray-100">
          ...
          <th className="text-left py-2 pr-3 font-semibold text-gray-600 print:text-gray-800 border-b border-gray-300">Violações Westgard</th>
        </tr>
      </thead>
      <tbody>
        {sorted.map((run, i) => {
          const allViolations = [...new Set(run.results.flatMap((r) => r.violations))] as WestgardViolation[];
          const hasRejection  = allViolations.some((v) => REJECTION_RULES.includes(v));
          return (
            <tr key={run.id} className={`border-t border-gray-200 print:border-gray-300
              ${hasRejection ? 'bg-red-50/50 print:bg-white' : allViolations.includes('1-2s') ? 'bg-amber-50/40 print:bg-white' : ''}`}>
              <td className="py-1.5 pl-3 font-mono text-gray-500 print:text-gray-600">{sorted.length - i}</td>
              <td className="py-1.5 font-mono text-gray-700 print:text-black whitespace-nowrap">{fmtFull(new Date(run.timestamp))}</td>
              <td className="py-1.5 text-gray-500 font-mono print:text-gray-600">{run.sampleId ?? '—'}</td>
              <td className="py-1.5 text-center">
                <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border
                  ${run.status === 'Aprovada' ? 'bg-green-50 text-green-700 border-green-200'
                    : run.status === 'Rejeitada' ? 'bg-red-50 text-red-700 border-red-200'
                    : 'bg-amber-50 text-amber-700 border-amber-200'}`}>{run.status}</span>
              </td>
  ...
  return (
    <div className="grid grid-cols-5 gap-0 border border-gray-200 rounded-lg overflow-hidden print:border-gray-300">
      {[
        ...
        <div key={i} className={`px-3 py-2.5 text-center ${i > 0 ? 'border-l border-gray-200 print:border-gray-300' : ''}`}>
  ...
  return (
    <div className={isFirst ? '' : 'print-level-break mt-12 print:mt-0'}>
      ...
      <div className="flex items-center gap-3 mb-5 print-avoid-break">
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${colors.bg} ${colors.text} ${colors.border}`}>
      ...
      <section className="mb-5 print-avoid-break">
        ...
        <div className="grid grid-cols-3 gap-0 border border-gray-200 rounded-lg overflow-hidden print:border-gray-300">
          {[
            ...
            <div key={i} className={`px-4 py-3 ${i > 0 ? 'border-l border-gray-200 print:border-gray-300' : ''} ${i >= 3 ? 'border-t border-gray-200 print:border-gray-300' : ''}`}>
              <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-0.5">{label}</p>
              <p className={`text-sm font-semibold print:text-black ${cls ?? 'text-gray-800'}`}>{value}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 print:bg-gray-100">
                <th className="text-left py-2 pl-3 font-semibold text-gray-600 border-b border-gray-200 print:border-gray-300">Analito</th>
                <th className="text-center py-2 font-semibold text-gray-600 border-b border-gray-200 print:border-gray-300">Unidade</th>
                <th className="text-center py-2 font-semibold text-gray-500 border-b border-gray-200 print:border-gray-300" colSpan={3}>← Fabricante →</th>
                <th className="text-center py-2 font-semibold text-gray-500 border-b border-gray-200 print:border-gray-300 border-l border-gray-200" colSpan={4}>← Interna (calculada) →</th>
              </tr>
              <tr className="bg-gray-50 print:bg-gray-100">
                ...
                <th className="pb-2 text-center text-[10px] font-medium text-gray-500 border-l border-gray-100">x̄</th>
                <th className="pb-2 text-center text-[10px] font-medium text-gray-500">SD</th>
                <th className="pb-2 text-center text-[10px] font-medium text-gray-500">CV%</th>
            ...
            <tbody>
              {analyteIds.map((id) => <AnalyteStatsRow key={id} analyteId={id} lot={lot} />)}
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section className="mb-5">
        ...
        <div className="space-y-4">
          {analyteIds.map((id) => {
            ...
            if (!analyte || !mfr) return null;
            return (
              <div key={id} className="border border-gray-200 rounded-lg overflow-hidden print:border-gray-300 print-avoid-break">
                <div className="flex items-center justify-between px-4 py-2 bg-gray-50 print:bg-gray-100 border-b border-gray-200 print:border-gray-300">
                  ...
                  <span className="text-[10px] text-gray-500">
                    {analyte.unit} · x̄ = {mfr.mean.toFixed(analyte.decimals)} · SD = {mfr.sd.toFixed(analyte.decimals)}
                  </span>
                </div>
...
export function ReportsView() {
  ...
  const refMonth = useMemo(() => {
    ...
    return referenceLots[0].startDate.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
  }, [referenceLots]);
  ...
  const totals = useMemo(() => {
    ...
    const pending  = total - approved - rejected;
    return { total, approved, rejected, pending, rate: total > 0 ? (approved / total * 100).toFixed(1) : '—' };
  }, [referenceLots]);
    ...
    <>
      ...
      <div id="cq-report"
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              {referenceLots.map((l) => {
                ...
                return (
                  <span key={l.id} className={`text-[10px] font-bold px-2 py-0.5 rounded border ${c.bg} ${c.text} ${c.border}`}>
                    NV{lv}
                  </span>
                );
              })}
              <span className="text-sm font-semibold text-gray-700 capitalize ml-1">{refMonth}</span>
            </div>
          </div>
          <div className="text-right shrink-0 ml-8">
            <p className="text-xs text-gray-500">Emitido por</p>
            <p className="text-sm font-semibold text-gray-800">{user?.displayName ?? user?.email ?? 'Operador'}</p>
        ...
        <section className="mb-8 print-avoid-break">
          <h2 className="text-[10px] font-semibold uppercase tracking-widest text-gray-500 mb-3">
            Resumo Consolidado — {referenceLots.length} nível{referenceLots.length !== 1 ? 'eis' : ''}
          </h2>
          <div className="grid grid-cols-5 gap-0 border border-gray-200 rounded-lg overflow-hidden print:border-gray-300">
            {[
              ...
              <div key={i} className={`px-4 py-3 text-center ${i > 0 ? 'border-l border-gray-200 print:border-gray-300' : ''}`}>
          ...
          <div className="flex items-start gap-4">
            <div className="shrink-0 w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center print:border print:border-gray-300">
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden>
                <path d="M8 1L2 4v4c0 4 2.5 6 6 7 3.5-1 6-3 6-7V4L8 1z" stroke="#6b7280" strokeWidth="1.3" fill="none" />
                <path d="M5.5 8l2 2 3-3" stroke="#6b7280" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <div className="flex-1">
              ...
              <p className="text-[10px] text-gray-500 leading-relaxed">
                ...
                Os dados pessoais presentes neste relatório são tratados exclusivamente para fins de rastreabilidade,
                controle de qualidade e cumprimento de obrigações regulatórias. Retenção documental: mínimo 5 anos.
              </p>
              <div className="flex items-center gap-6 mt-2 pt-2 border-t border-gray-200">
                <p className="text-[9px] text-gray-400">Sistema: CQ Hematologia Labclin · RDC 978/2025 compliant</p>
                <p className="text-[9px] text-gray-400 ml-auto">Gerado em: {fmtFull(generatedAt)}</p>
              </div>
            </div>
          </div>
        </footer>
