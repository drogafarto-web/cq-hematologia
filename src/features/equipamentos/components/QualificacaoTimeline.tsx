...
const RESULTADO_STYLE: Record<QualificacaoResultado, { bg: string; text: string; border: string }> =
  {
    ...
    pendente: {
      bg: 'bg-slate-500/10',
      text: 'text-slate-500 dark:text-white/40',
      border: 'border-slate-300 dark:border-white/[0.12]',
    },
  };
...
export function QualificacaoTimeline({
  labId,
  equipamentoId,
  canMutate = true,
}: QualificacaoTimelineProps) {
    ...
    <div className="space-y-4">
        ...
        {ETAPAS.map((etapa, idx) => {
          ...
          return (
            <div key={etapa} className="flex-1 relative">
              ...
              {idx < ETAPAS.length - 1 && (
                <div
                  className={`absolute top-5 left-1/2 w-full h-0.5 ${qual?.resultado === 'aprovado' ? 'bg-emerald-400' : 'bg-slate-200 dark:bg-white/[0.08]'}`}
                />
              )}
              ...
              <div className="relative flex flex-col items-center text-center px-2">
                {/* Circle */}
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${style.border} ${style.bg} z-10`}
                >
                  {resultado === 'aprovado' && (
                    <svg
                ...
                {/* Label */}
                <p className={`mt-2 text-xs font-semibold ${style.text}`}>{etapa}</p>
      ...
      <div className="bg-slate-50 dark:bg-white/[0.03] rounded-xl p-3 border border-slate-200 dark:border-white/[0.06]">
        <p className="text-[11px] text-slate-500 dark:text-white/40 leading-relaxed">
          <strong>IQ:</strong> {QUALIFICACAO_ETAPA_DESC.IQ}
          <br />
          <strong>OQ:</strong> {QUALIFICACAO_ETAPA_DESC.OQ}
          <br />
        ...
        <div className="space-y-2">
          ...
          {qualificacoes.map((q) => {
            ...
            return (
              <div
                key={q.id}
                className={`flex items-center gap-3 p-2.5 rounded-lg border ${s.border} ${s.bg}`}
              >
                <span className={`text-xs font-bold ${s.text}`}>{q.etapa}</span>
                <span className={`text-xs ${s.text}`}>
                  {QUALIFICACAO_RESULTADO_LABEL[q.resultado]}
                </span>
