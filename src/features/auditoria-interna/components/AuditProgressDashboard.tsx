    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex-1 grid grid-cols-2 gap-3">
          <StatCard icon={<CheckCircle2 className="w-4 h-4 text-emerald-400" />} label="Conforme" value={stats.conforme} color="text-emerald-400" />
          <StatCard icon={<XCircle className="w-4 h-4 text-red-400" />} label="NC" value={stats.nc} color="text-red-400" />
          <StatCard icon={<MinusCircle className="w-4 h-4 text-white/40" />} label="N/A" value={stats.na} color="text-white/40" />
          <StatCard icon={<Clock className="w-4 h-4 text-amber-400" />} label="Pendente" value={stats.pendente} color="text-amber-400" />
        </div>
      </div>

      {/* Trend */}
      {trend !== null && (
        <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${trend >= 0 ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}>
          ...
          <span className={`text-sm font-medium ${trend >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            {trend >= 0 ? '+' : ''}{trend.toFixed(1)}% vs auditoria anterior
          </span>
        </div>
      )}
      ...
      <div>
        <div className="flex justify-between text-xs text-white/50 mb-1">
          <span>{stats.respondidos}/{stats.total} itens respondidos</span>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {Object.entries(stats.byBloco).sort(([a], [b]) => a.localeCompare(b)).map(([bloco, data]) => {
              ...
              <div key={bloco} className="flex items-center gap-2 p-2 rounded-lg bg-white/5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-white/60">{Math.round(blocoScore)}%</span>
                  </div>
                </div>
              </div>
            );
