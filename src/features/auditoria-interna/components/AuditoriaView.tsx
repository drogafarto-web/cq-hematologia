...
export function AuditoriaView() {
  ...
  const [currentTab, setCurrentTab] = useState<'planejamento' | 'em-execucao' | 'historico'>('em-execucao');
  ...
  const [activeSessao, setActiveSessao] = useState<{ auditoriaId: string; sessaoId: string } | null>(null);
      ...
      <div className="min-h-screen bg-[#141417] text-white/90 p-4 sm:p-8 space-y-8 animate-in fade-in duration-500">
        <header className="flex flex-col sm:flex-row sm:items-center justify-between max-w-7xl mx-auto gap-4">
          ...
          <div className="flex items-center gap-3">
             <h1 className="text-xl font-bold">Ciclo {audit?.ano} — {audit?.frequencia}</h1>
            <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
              audit?.status === 'em_execução' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
            }`}>
          ...
          <div className="space-y-8">
            <section className="bg-gradient-to-br from-violet-600/10 to-fuchsia-600/10 border border-white/10 rounded-3xl p-6 sm:p-8 shadow-xl">
              ...
              <SessoesList 
                ...
                onSelectSessao={(audId, sesId) => setActiveSessao({ auditoriaId: audId, sessaoId: sesId })}
              />
            </section>
            ...
            <section className="bg-white/5 border border-white/10 rounded-3xl p-6 sm:p-8">
              ...
              <div className="space-y-3">
                <button className="w-full text-left px-5 py-3 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/5 transition text-sm font-medium">Exportar Relatório PDF</button>
                <button className="w-full text-left px-5 py-3 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/5 transition text-sm font-medium">Gerenciar Auditores</button>
                <button className="w-full text-left px-5 py-3 rounded-2xl bg-red-500/5 hover:bg-red-500/10 border border-red-500/10 text-red-400 text-sm font-medium">Cancelar Auditoria</button>
              </div>
            </section>
          </div>
        </main>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8 sm:py-12">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="text-white/40 text-base mt-2 font-medium">Módulo de Gestão da Qualidade (DICQ/ISO)</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-10">
        ...
        {currentTab === 'em-execucao' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {auditorias.filter(a => a.status === 'em_execução').map(audit => (
              <AuditCard 
                key={audit.id} 
                audit={audit} 
            ...
            {auditorias.filter(a => a.status === 'em_execução').length === 0 && (
              <div className="col-span-full py-32 text-center border-2 border-dashed border-white/5 rounded-[40px] bg-white/[0.01]">
                ...
                <p className="text-white/40 text-lg font-medium">Nenhuma auditoria ativa no momento.</p>
                <p className="text-white/20 text-sm mt-1">Vá em Planejamento para iniciar um novo ciclo.</p>
              </div>
            )}
          </div>
        )}
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-8">
          ...
          <div className="text-right">
            <p className="text-[10px] text-white/30 uppercase tracking-[0.2em] font-black">Ciclo Ativo</p>
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center text-[11px] font-bold text-white/30 uppercase tracking-widest">
            <span>{audit.tipoExecucao === 'reAuditoria' ? 'Re-auditoria' : 'Auditoria Inicial'}</span>
