...
export function CEQPdfUpload() {
  ...
  const handleFiles = useCallback((fileList: FileList | null) => {
    ...
    if (oversized.length > 0) {
      setState({
        step: 'error',
        message: `Arquivo(s) excede(m) 20MB: ${oversized.map((f) => f.name).join(', ')}`,
      });
      return;
    }
    setFiles(pdfs);
  ...
  const handleUploadAndParse = async () => {
    ...
    try {
      ...
      const parseFn = httpsCallable<
        { labId: string; storagePaths: string[] },
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!validated.success) {
        setState({
          step: 'error',
          message: `Dados inválidos: ${validated.error.issues[0]?.message}`,
        });
        return;
      }
      ...
      const confirmFn = httpsCallable<
        typeof payload,
        { amostras: number; resultados: number; ncsGeradas: number }
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <p className="text-sm text-white/50">
              Arraste PDFs do PNCQ aqui ou clique para selecionar
            </p>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {files.map((f, i) => (
                  <span
                    key={i}
                    className="text-[11px] px-2 py-1 rounded bg-white/[0.05] text-white/60 border border-white/[0.08]"
                  >
                    {f.name}
                  </span>
                ))}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-white/70 font-medium">
              {state.reports.length} relatório(s) ·{' '}
              {state.reports.reduce((s, r) => s + r.resultados.length, 0)} resultado(s)
            </p>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={reset}
                className="text-xs text-white/40 hover:text-white/60 px-3 py-1.5 rounded border border-white/10 transition-colors"
              >
                Cancelar
              </button>
              <button
                ...
                className="text-xs text-white px-3 py-1.5 rounded bg-teal-500 hover:bg-teal-400 font-medium transition-colors"
              >
                Confirmar e gravar
              </button>
            </div>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {state.reports.map((r, i) => (
              <div
                key={i}
                className="bg-white/[0.03] border border-white/[0.05] rounded-lg px-4 py-3"
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-white/80 font-medium capitalize">
                      {r.especialidade.replace(/-/g, ' ')}
                    </p>
                    <p className="text-[11px] text-white/35">
                      {r.mesAno} · Lote {r.loteProEx}
                    </p>
                  </div>
      ...
      {state.step === 'done' && (
        <div className="text-center py-8">
          ...
          <p className="text-[11px] text-white/40 mt-1">
            ...
            {state.ncsGeradas > 0 && (
              <span className="text-red-400"> · {state.ncsGeradas} NC(s) gerada(s)</span>
            )}
          </p>
          <button
            type="button"
            onClick={reset}
            className="mt-4 text-xs text-teal-400 hover:text-teal-300 transition-colors"
          >
