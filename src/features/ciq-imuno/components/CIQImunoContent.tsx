...
function PlusIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden><path d="M12 5v14M5 12h14"/></svg>;
}
function DownloadIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>;
}
function XIcon() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden><path d="M18 6 6 18M6 6l12 12"/></svg>;
}
function PrintIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2"/><path d="M6 14h12v8H6z"/></svg>;
}
function CheckIcon() {
  return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M20 6L9 17l-5-5"/></svg>;
}
function BanIcon() {
  return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" aria-hidden><circle cx="12" cy="12" r="10"/><path d="M4.9 4.9l14.2 14.2"/></svg>;
}
function QRIcon() {
  return (
    <svg width="13" height="13" viewBox="0 0 14 14" fill="none" aria-hidden>
      ...
      <path d="M8 8h1.5v1.5H8zM9.5 9.5H11v1.5H9.5zM11 8h1.5v1.5H11zM8 11h1.5v1.5H8zM11 11h1.5v1.5H11z" fill="currentColor"/>
    </svg>
  );
}
function EditIcon() {
  return <svg width="13" height="13" viewBox="0 0 13 13" fill="none" aria-hidden><path d="M9 2l2 2-7 7H2v-2l7-7z" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>;
}
function TrashIcon() {
  return <svg width="13" height="13" viewBox="0 0 13 13" fill="none" aria-hidden><path d="M2 3.5h9M4.5 3.5V2.5a.5.5 0 01.5-.5h3a.5.5 0 01.5.5v1M5 6v4M8 6v4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/><path d="M3 3.5l.5 7a.5.5 0 00.5.5h5a.5.5 0 00.5-.5l.5-7" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>;
}
function SpinnerIcon() {
  return <svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none" aria-hidden><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeOpacity="0.25"/><path d="M22 12a10 10 0 00-10-10" stroke="currentColor" strokeWidth="3" strokeLinecap="round"/></svg>;
}
function ImunoIcon() {
...
function ValidityBanner({ validadeControle, runs }: { validadeControle: string; runs: CIQImunoRun[] }) {
  ...
  if (controle.level === 'expired')
    msgs.push(`Controle expirado há ${Math.abs(controle.daysLeft)} dia(s). Não registre mais corridas neste lote — substitua o material de controle.`);
  ...
  if (reagenteWorst.level === 'expired' && reagenteWorst.lote)
    msgs.push(`Kit reagente lote ${reagenteWorst.lote} vencido (${reagenteWorst.val}). Não use em corridas novas.`);
...
function RunRow({ run, onShowQR }: { run: CIQImunoRun; onShowQR: (r: CIQImunoRun) => void }) {
  ...
  const conformityCls = conforme
    ...
    : 'bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 border-red-200 dark:border-red-500/20';
  const neutralCls = 'bg-slate-100 dark:bg-white/[0.05] text-slate-700 dark:text-white/70 border-slate-200 dark:border-white/10';
  ...
  return (
    <tr className="border-t border-slate-100 dark:border-white/[0.04] hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
      <td className="px-4 py-3 font-mono text-xs text-slate-400 dark:text-slate-500 hidden md:table-cell">{run.runCode ?? '—'}</td>
      ...
      <td
        ...
        title={run.fabricanteReagente ? `${run.fabricanteReagente} · ${run.loteReagente}` : run.loteReagente}
      >
        {run.loteReagente ?? '—'}
      </td>
      ...
      <td className="px-4 py-3">
        <span
          ...
          title={conforme ? 'Conforme — resultado obtido bateu com o esperado' : 'Não conforme — obtido diverge do esperado. Ação corretiva obrigatória.'}
        >
          {conforme ? '✓' : '✗'}
      ...
      <td className="px-4 py-3 text-xs">
        {hasAlerts
          ? <span className="text-amber-600 dark:text-amber-400">{run.westgardCategorico!.length} alerta{run.westgardCategorico!.length !== 1 ? 's' : ''}</span>
          : <span className="text-slate-300 dark:text-white/20">—</span>
        }
      </td>

      {/* Assinatura — nome do operador + hash curto */}
      <td className="px-4 py-3 text-xs hidden sm:table-cell" title={run.logicalSignature ? `${run.operatorName}\nSHA-256: ${run.logicalSignature}` : run.operatorName ?? ''}>
        <div className="flex flex-col gap-0.5">
          <span className="text-slate-600 dark:text-white/60 truncate max-w-[140px]">{operatorShort}</span>
          {run.logicalSignature && (
            <span className="font-mono text-[10px] text-slate-400 dark:text-slate-500">{run.logicalSignature.slice(0, 8)}…</span>
          )}
        </div>
      </td>

      <td className="px-4 py-3 text-right">
        <button type="button" onClick={() => onShowQR(run)} aria-label="Ver QR Code"
...
function WestgardAlertsPanel({ runs }: { runs: CIQImunoRun[] }) {
  ...
  return (
    <div className={`rounded-xl border px-4 py-3.5 space-y-2 ${isRejected ? 'bg-red-50 dark:bg-red-500/[0.06] border-red-200 dark:border-red-400/20' : 'bg-amber-50 dark:bg-amber-500/[0.06] border-amber-200 dark:border-amber-500/20'}`}>
      <p className={`text-xs font-semibold ${isRejected ? 'text-red-600 dark:text-red-400' : 'text-amber-700 dark:text-amber-400'}`}>
      ...
      <ul className="space-y-1">
        {alerts.map((a) => (
          <li key={a} className="text-xs text-slate-600 dark:text-white/55 flex items-start gap-2">
            <span className={`mt-0.5 shrink-0 ${isRejected ? 'text-red-400' : 'text-amber-400'}`}>›</span>
            {ALERT_LABELS[a] ?? a}
          </li>
        ))}
      </ul>
...
function RowDetail({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex justify-between gap-4">
      ...
      <span className={`text-slate-700 dark:text-white/70 text-right ${mono ? 'font-mono' : ''}`}>{value}</span>
    </div>
  );
}

function EditLotForm({ lot, isSaving, error, onSave, onCancel }: {
  lot: CIQImunoLot; isSaving: boolean; error: string | null;
  ...
  const inputCls = 'w-full h-9 px-3 rounded-lg text-sm bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.10] text-slate-800 dark:text-white/85 outline-none focus:border-blue-400 dark:focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/10 transition-all';
  const labelCls = 'block text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-white/40 mb-1.5';
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave({ aberturaControle: abertura, validadeControle: validade }); }} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input id="edit-abertura" type="date" value={abertura} onChange={(e) => setAbertura(e.target.value)} required className={inputCls}/>
        </div>
        <div>
          <label htmlFor="edit-validade" className={labelCls}>Data de validade</label>
          <input id="edit-validade" type="date" value={validade} onChange={(e) => setValidade(e.target.value)} required className={inputCls}/>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-2 pt-1">
        ...
        <button type="submit" disabled={isSaving}
          ...
          {isSaving ? <><SpinnerIcon/> Salvando…</> : 'Salvar alterações'}
        </button>
      </div>
    </form>
  );
...
export function CIQImunoContent({ lots, activeLotId, setActiveLotId, newRunTrigger }: CIQImunoContentProps) {
  const user        = useUser();
  ...
  useEffect(() => {
    if (newRunTrigger > 0) { setFormError(null); setShowForm(true); }
  }, [newRunTrigger]);
  ...
  async function handleDeleteLot() {
    ...
    setIsDeleting(true);
    try {
      await deleteCIQLot(deletingLot.labId, deletingLot.id, {
      ...
      <>
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 dark:text-white/30 mt-1">Registre a primeira corrida para criar um lote de controle.</p>
          </div>
          <button type="button" onClick={() => { setFormError(null); setShowForm(true); }}
        ...
        {showForm && (
          <Modal title="Registrar corrida" onClose={() => setShowForm(false)}>
            ...
            <CIQImunoForm onSave={handleSave} isSaving={isSaving} onCancel={() => setShowForm(false)}/>
          </Modal>
        )}
      </>
    );
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-end gap-4 mb-2">
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{activeLot.testType}</h1>
          ...
          <div className="ml-auto flex items-center gap-2 flex-wrap justify-end">
            {canDecide && runs.length > 0 && activeLot.ciqDecision !== 'A' && (
              <button type="button" onClick={() => handleLotDecision('A')} disabled={decidingLot}
            ...
            {canDecide && runs.length > 0 && activeLot.ciqDecision !== 'Rejeitado' && activeLot.ciqDecision !== 'A' && (
              <button type="button" onClick={() => handleLotDecision('Rejeitado')} disabled={decidingLot}
            ...
            {canDecide && (
              <>
                <button type="button" onClick={() => { setSaveLotErr(null); setEditingLot(activeLot); }}
                ...
                <button type="button" onClick={() => { setDeleteErr(null); setDeletingLot(activeLot); }}
      ...
      {activeLot?.ciqDecision && (
        <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm ${
          ...
          <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${activeLot.ciqDecision === 'A' ? 'bg-emerald-500' : 'bg-red-500'}`}/>
      ...
      {activeLot && (
        <ValidityBanner validadeControle={activeLot.validadeControle} runs={runs}/>
      )}

      {/* Runs table */}
      {runsLoading ? (
        <div className="flex items-center justify-center py-12">
          <svg className="animate-spin w-4 h-4 text-slate-300 dark:text-white/20" viewBox="0 0 24 24" fill="none">
        ...
        <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.06] rounded-xl p-8 text-center shadow-sm">
          <p className="text-sm text-slate-400 dark:text-slate-500">Nenhuma corrida registrada neste lote.</p>
          <button type="button" onClick={() => { setFormError(null); setShowForm(true); }}
            ...
            <thead>
              <tr className="bg-slate-50 dark:bg-white/[0.02] border-b border-slate-200 dark:border-white/[0.06]">
                {[
                  ...
                  <th key={i} className={`px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 ${h.cls}`}>
                    {h.label}
                  </th>
                ))}
              </tr>
      ...
      {editingLot && (
        <Modal title="Editar lote" onClose={() => setEditingLot(null)}>
          <EditLotForm lot={editingLot} isSaving={isSavingLot} error={saveLotErr} onSave={handleEditLotSave} onCancel={() => setEditingLot(null)}/>
        </Modal>
      )}
        ...
        <Modal title="Excluir lote" onClose={() => !isDeleting && setDeletingLot(null)}>
          <div className="space-y-4">
            <div className="rounded-xl border border-red-200 dark:border-red-400/20 bg-red-50 dark:bg-red-500/[0.06] px-4 py-3.5">
              <p className="text-sm font-semibold text-red-700 dark:text-red-400">Esta operação é irreversível</p>
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button type="button" onClick={() => setDeletingLot(null)} disabled={isDeleting}
                className="h-9 px-4 rounded-lg text-sm text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/[0.10] hover:bg-slate-50 dark:hover:bg-white/[0.05] disabled:opacity-40 transition-all">
                Cancelar
              </button>
              <button type="button" onClick={handleDeleteLot} disabled={isDeleting}
                ...
                {isDeleting ? <><SpinnerIcon/> Excluindo…</> : <><TrashIcon/> Excluir lote</>}
              </button>
            </div>
          </div>
        </Modal>
        ...
        <Modal title="QR Code de auditoria" onClose={() => setQRRun(null)}>
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="w-full rounded-xl border border-slate-100 dark:border-white/[0.07] bg-slate-50 dark:bg-white/[0.03] p-4 space-y-2 text-xs">
              ...
              <RowDetail label="Resultado"  value={qrRun.resultadoObtido === 'R' ? 'Reagente' : 'Não Reagente'}/>
              <RowDetail label="Operador"   value={qrRun.operatorName}/>
              <RowDetail label="Assinatura" value={qrRun.logicalSignature ? `${qrRun.logicalSignature.slice(0, 16)}…` : '—'} mono/>
            </div>
          </div>
        </Modal>
      )}
