...
function LotCard({
  lot, isActive, canManage, onSelect, onEdit, onDelete,
}: {
  lot: CIQImunoLot; isActive: boolean; canManage: boolean;
    ...
    <div className={[
      ...
      <div className="relative pointer-events-none">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <p className="text-sm font-medium text-slate-800 dark:text-white/85 truncate">{lot.testType}</p>
            <p className="text-xs text-slate-400 dark:text-white/35 mt-0.5 font-mono truncate">{lot.loteControle}</p>
          </div>
        ...
        <div className="flex items-center gap-2 mt-2 text-[11px] text-slate-400 dark:text-white/30">
          <span>{lot.runCount} corrida{lot.runCount !== 1 ? 's' : ''}</span>
      ...
      {canManage && (
        <div className="absolute top-2.5 right-3 hidden group-hover:flex items-center gap-0.5 pointer-events-auto">
          <button type="button" aria-label="Editar lote" onClick={onEdit}
          ...
          <button type="button" aria-label="Excluir lote" onClick={onDelete}
 ...
 */
function notivisaBadgeState(run: CIQImunoRun): { label: string; cls: string; tooltip: string } | null {
  ...
  return (
    <tr className="border-t border-slate-100 dark:border-white/[0.04] hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
      ...
      <td className="px-4 py-3">
        <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium border ${resultCls}`}>
          <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${conforme ? 'bg-emerald-500' : 'bg-red-500'}`} />
          {run.resultadoObtido === 'R' ? 'R' : 'NR'}
          {!conforme && <span className="opacity-60 text-[10px]">(!)</span>}
        </span>
      </td>
      <td className="px-4 py-3">
        <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-bold border ${resultCls}`}>
          {conforme ? 'A' : 'NA'}
        </span>
      </td>
      ...
      <td className="px-4 py-3 text-right">
        <button type="button" onClick={() => onShowQR(run)} aria-label="Ver QR Code de auditoria"
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B0F14] text-slate-900 dark:text-white">
      ...
      <header className="h-14 bg-white dark:bg-[#0F1318] border-b border-slate-200 dark:border-white/[0.06] flex items-center gap-4 px-6 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2 text-[13px] text-slate-400 dark:text-slate-500">
          <button type="button" onClick={() => setCurrentView('hub')}
        ...
        <div className="max-w-[1400px] mx-auto px-8 py-6">
          <div className="flex gap-6 items-start">
            ...
            <main className="flex-1 min-w-0 space-y-4">
              ...
              {/* Mobile: back */}
              <button type="button" onClick={() => setActiveLotId(null)}
