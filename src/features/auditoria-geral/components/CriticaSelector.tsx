    ...
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-2">
        {OPTIONS.map((opt) => {
          ...
          let baseStyle = 'bg-slate-50 border-slate-200 text-slate-600 dark:bg-white/[0.04] dark:border-white/[0.08] dark:text-white/50';
          if (isSelected) {
            if (opt.value === 'CONFORME') {
              baseStyle = 'bg-emerald-100 border-emerald-400 text-emerald-800 dark:bg-emerald-500/20 dark:border-emerald-500/50 dark:text-emerald-300 ring-1 ring-emerald-500/30';
            } else if (opt.value === 'NÃO CONFORME') {
              baseStyle = 'bg-red-100 border-red-400 text-red-800 dark:bg-red-500/20 dark:border-red-500/50 dark:text-red-300 ring-1 ring-red-500/30';
            } else {
              baseStyle = 'bg-slate-200 border-slate-400 text-slate-700 dark:bg-white/[0.08] dark:border-white/20 dark:text-white/70 ring-1 ring-slate-400/30';
            }
          } else if (isSuggested) {
            if (opt.value === 'CONFORME') {
              baseStyle = 'bg-slate-50 border-emerald-300 text-slate-600 dark:bg-white/[0.04] dark:border-emerald-500/30 dark:text-white/50 border-dashed';
            } else if (opt.value === 'NÃO CONFORME') {
              baseStyle = 'bg-slate-50 border-red-300 text-slate-600 dark:bg-white/[0.04] dark:border-red-500/30 dark:text-white/50 border-dashed';
            }
          }
