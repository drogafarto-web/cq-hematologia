...
const STATUS_COLORS: Record<PlanoAcao['status'], string> = {
  pendente:
    'bg-amber-100 text-amber-700 border-amber-300 dark:bg-amber-400/10 dark:text-amber-400 dark:border-amber-400/20',
  em_andamento:
    'bg-violet-100 text-violet-700 border-violet-300 dark:bg-violet-500/10 dark:text-violet-400 dark:border-violet-500/20',
  concluido:
    ...
    return (
      <div className="text-center py-10">
        <p className="text-sm text-emerald-600 dark:text-emerald-400">
          Nenhum indicador critico (score &le; 2). Plano de acao nao necessario.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-slate-600 dark:text-white/60">
          Plano de Acao — {criticos.length} indicador{criticos.length > 1 ? 'es' : ''} critico
          {criticos.length > 1 ? 's' : ''}
        </h3>
      ...
      <div className="space-y-3">
        {criticos.map((r) => {
          ...
          return (
            <div
              key={r.id}
              className="bg-white border border-slate-200 dark:bg-white/[0.02] dark:border-white/[0.06] rounded-lg p-4"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {plano && !isEditing && (
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ${STATUS_COLORS[plano.status]}`}
                  >
              ...
              {plano && !isEditing && (
                <div className="mt-3 pt-3 border-t border-slate-100 dark:border-white/[0.04] space-y-1">
                  <p className="text-xs text-slate-600 dark:text-white/60">
                    <span className="text-slate-400 dark:text-white/30">Acao:</span>{' '}
                    {plano.acaoCorretiva}
                  </p>
                  <p className="text-xs text-slate-600 dark:text-white/60">
                    <span className="text-slate-400 dark:text-white/30">Responsavel:</span>{' '}
                    {plano.responsavel}
                  </p>
                  {plano.prazo && (
                    <p className="text-xs text-slate-600 dark:text-white/60">
                      <span className="text-slate-400 dark:text-white/30">Prazo:</span>{' '}
                      {plano.prazo.toDate().toLocaleDateString('pt-BR')}
                    </p>
                  )}
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    {plano.status === 'pendente' && (
                      <button
                        onClick={() => handleStatusChange(r, 'em_andamento')}
                    ...
                    {plano.status === 'em_andamento' && (
                      <button
                        onClick={() => handleStatusChange(r, 'concluido')}
