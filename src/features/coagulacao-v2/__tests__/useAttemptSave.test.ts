  ...
  useCoagSignature: () => ({
    sign: vi.fn(() =>
      Promise.resolve({
        ...
        signedBy: 'user-001',
        signedAt: '2026-05-25T01:00:00.000Z',
      }),
    ),
    isReady: true,
  }),
...
vi.mock('../../coagulacao/hooks/useCoagWestgard', () => ({
  computeCoagWestgard: () => ({
    byRun: new Map([
      ['__simulated__', { conformidade: 'A', allViolations: [], analitosComViolacao: [] }],
    ]),
    lotStatus: 'valido',
    lotAlerts: [],
  }),
    ...
    vi.mocked(await import('../../coagulacao/hooks/useCoagWestgard')).computeCoagWestgard = vi.fn(
      () => ({
        byRun: new Map([
          [
            '__simulated__',
            { conformidade: 'R', allViolations: [{ rule: '1-2s' }], analitosComViolacao: ['rni'] },
          ],
        ]),
        lotStatus: 'valido',
        lotAlerts: [],
