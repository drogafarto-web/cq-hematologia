...
function resolveAnalyteId(raw: string): string | null {
  ...
  const direct = ANALYTES.find((a) => normaliseKey(a.id) === key || normaliseKey(a.name) === key);
 ...
 */
export function parseControlLotCSV(text: string, targetLevel?: 1 | 2 | 3): ParsedCSVResult {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const hasMeanCol = norm.some(
      (c) => c.includes('mean') || c.includes('média') || c.includes('media'),
    );
    ...
    if (isMultiLevel) {
      ...
      if (meanCol === -1) {
        result.warnings.push(`Nível ${tl} não encontrado. Usando primeiro nível disponível.`);
...
export function statsToManufacturerStats(stats: ParsedStat[]): ManufacturerStats {
  return Object.fromEntries(stats.map(({ analyteId, mean, sd }) => [analyteId, { mean, sd }]));
}
...
export function parseMultiLevelLotCsv(csvText: string): ParsedMultiLevelResult {
  ...
  if (missing.length > 0) {
    throw new Error(`CSV inválido: nível(is) obrigatório(s) ausente(s): ${missing.join(', ')}.`);
  }
