...
function RunDot({ cx, cy, payload }: DotProps) {
  ...
  const color = payload.isRejection
    ? '#f87171' // red-400
    : payload.isWarningOnly
      ? '#fbbf24' // amber-400
      : payload.status === 'Aprovada'
        ? '#34d399' // emerald-400
        : isDark
          ? '#94a3b8'
          : '#64748b'; // slate-400 (Pendente/no value)
...
function CustomTooltip({ active, payload, analyte }: TooltipProps) {
  ...
  const statusColor = p.isRejection
  ...
  return (
    <div className="rounded-xl bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-white/[0.1] px-4 py-3 shadow-xl text-xs transition-colors duration-300">
      <p className="text-slate-500 dark:text-white/40 mb-2">
        {dateStr} {timeStr}
      </p>
      <p className="text-slate-900 dark:text-white/90 font-semibold text-sm">
        {p.value.toFixed(analyte.decimals)}{' '}
        <span className="text-slate-400 dark:text-white/40 font-normal text-xs">
          {analyte.unit}
        </span>
      </p>
      {p.zScore !== null && (
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 rounded-2xl border border-slate-200 dark:border-white/[0.07] bg-white dark:bg-white/[0.01] transition-colors duration-300">
        ...
        <p className="text-xs text-slate-300 dark:text-white/20 mt-1">
          Adicione corridas para gerar o gráfico
        </p>
      </div>
    );
  }
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={data} margin={{ top: 12, right: 30, left: 0, bottom: 4 }}>
            ...
            <ReferenceLine
              ...
              strokeDasharray="4 3"
              label={<RefLabel text="+3s" color="#f87171" />}
            />
            <ReferenceLine
              y={stats.minus3sd}
              stroke="rgba(248,113,113,0.45)"
            ...
            <ReferenceLine
              ...
              strokeDasharray="5 3"
              label={<RefLabel text="+2s" color="#fbbf24" />}
            />
            <ReferenceLine
              y={stats.minus2sd}
              stroke="rgba(251,191,36,0.40)"
            ...
            <ReferenceLine
              ...
              strokeWidth={1.5}
              label={
                <RefLabel text="x̄" color={isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.6)'} />
              }
            />
              ...
              return (
                <div key={i} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <span className="text-xs text-slate-600 dark:text-white/50">
                    Corrida #{alert.runIndex} — {alert.value.toFixed(analyte.decimals)}{' '}
                    {analyte.unit}
                  </span>
