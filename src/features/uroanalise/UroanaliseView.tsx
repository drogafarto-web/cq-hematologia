...
const BackIcon      = () => <svg width="13" height="13" viewBox="0 0 13 13" fill="none" aria-hidden><path d="M8 2L3 6.5 8 11" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>;
const UrineIcon     = () => <svg width="15" height="15" viewBox="0 0 20 20" fill="none" aria-hidden><path d="M10 2.5C10 2.5 5 9 5 13a5 5 0 0 0 10 0C15 9 10 2.5 10 2.5z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/></svg>;
const DotsIcon      = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden><circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/></svg>;
const SmallPlusIcon = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden><path d="M12 5v14M5 12h14"/></svg>;
const ReportIcon    = () => <svg width="13" height="13" viewBox="0 0 14 14" fill="none" aria-hidden><rect x="2" y="1" width="10" height="12" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><path d="M4.5 4.5h5M4.5 7h5M4.5 9.5h3" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>;

const STATUS_DOT: Record<UroLotStatus, string> = {
  sem_dados: 'bg-slate-400', valido: 'bg-emerald-500', atencao: 'bg-amber-500', reprovado: 'bg-red-500',
};
...
function Sidebar({ activeLab, userName, userRole, signOut, lots, activeLotId, setActiveLotId, onNewRun, setCurrentView }: SidebarProps) {
    ...
    <aside className="w-[232px] shrink-0 bg-white dark:bg-[#0F1318] border-r border-slate-200 dark:border-white/[0.06] flex flex-col h-screen sticky top-0 overflow-hidden">
      <div className="flex items-center gap-2.5 px-4 py-5 border-b border-slate-100 dark:border-white/[0.05]">
        ...
        <div className="min-w-0">
          ...
          {activeLab && <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 truncate max-w-[140px]">{activeLab.name}</div>}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-3 px-2">
        ...
        <div className="flex items-center gap-2 px-2.5 py-2 mb-1">
          <span className="text-amber-500 dark:text-amber-400"><UrineIcon/></span>
          <span className="text-[13px] font-semibold text-slate-700 dark:text-white/80">Uroanálise</span>
          <span className="ml-auto font-mono text-[9px] px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20">CLSI GP16</span>
        </div>
          ...
          <>
            ...
            <div className="space-y-1">
              {lots.map((lot) => {
                ...
                return (
                  <button key={lot.id} type="button" onClick={() => setActiveLotId(lot.id)}
                    className={[
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <span className={`text-[13px] font-medium truncate ${isActive ? 'text-amber-800 dark:text-amber-300' : 'text-slate-700 dark:text-white/75'}`}>
                        Nível {lot.nivel}
                      </span>
                      <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${STATUS_DOT[lot.lotStatus]}`}/>
                    </div>
      ...
      <div className="border-t border-slate-100 dark:border-white/[0.05] p-3">
        <div className="flex items-center gap-2.5 px-1 py-1 rounded-lg hover:bg-slate-50 dark:hover:bg-white/[0.04] transition-colors cursor-default">
          <div className="w-7 h-7 rounded-full bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 flex items-center justify-center text-[11px] font-semibold shrink-0">{initials}</div>
          <div className="min-w-0">
          ...
          <button type="button" onClick={signOut} className="ml-auto p-1 text-slate-300 dark:text-slate-600 hover:text-slate-500 dark:hover:text-slate-400 transition-colors" title="Sair">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" aria-hidden><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9"/></svg>
          </button>
        </div>
      </div>
    </aside>
...
function Topbar({ activeLab, userEmail, setCurrentView, isSuperAdmin, onOpenSettings }: {
  activeLab: { name: string } | null;
  userEmail: string;
  setCurrentView: (v: View) => void;
...
export function UroanaliseView() {
  ...
  useEffect(() => {
    if (!activeLotId && lots.length > 0) setActiveLotId(lots[0].id);
  }, [activeLotId, lots]);
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-[#0B0F14] text-slate-900 dark:text-white overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <Topbar activeLab={activeLab} userEmail={user?.email ?? ''} setCurrentView={setCurrentView} isSuperAdmin={isSuperAdmin} onOpenSettings={() => setShowSettings(true)}/>
