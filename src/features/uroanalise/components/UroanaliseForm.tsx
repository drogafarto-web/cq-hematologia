...
const INPUT_ERR = INPUT.replace('border-slate-200 dark:border-white/[0.09]', 'border-red-400/60 dark:border-red-400/40');
...
function SectionTitle({ children, hint }: { children: React.ReactNode; hint?: string }) {
  return (
    <div className="mb-3 flex items-baseline justify-between">
      <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-white/30">{children}</p>
...
function ButtonGrid({ analyte, value, origem, ocrConf, options, isConforme, onPick }: ButtonGridProps) {
    ...
    <div className="space-y-1.5">
      ...
      {isOcrSuggested && ocrConf !== undefined && (
        <p className={`text-[10px] font-mono ${
          ocrConf >= 0.85 ? 'text-sky-600 dark:text-sky-400' : 'text-amber-600 dark:text-amber-400'
        }`}>
    ...
    <div className="rounded-xl border border-amber-400/25 bg-amber-500/[0.04] p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-amber-900 dark:text-amber-300">Leitura assistida por IA — experimental</p>
          <p className="text-xs text-amber-700/80 dark:text-amber-400/70 mt-0.5">
            Fotografe a tira contra o padrão de cores. A IA preenche campos com alta confiança; revise TODOS os valores antes de confirmar.
            Bilirrubina, Urobilinogênio e Densidade são sempre manuais (contraste ótico insuficiente).
          </p>
          <div className="mt-3">
            <label htmlFor="ocr-input" className={[
...
export function UroanaliseForm({
  onSave, isSaving = false, onCancel, initialNivel, ocrEnabled = false, labId = null,
}: UroanaliseFormProps) {
  const user = useUser();
  ...
  function setCategorico(id: UroAnalitoId, valor: UroValorCategorico) {
    setForm((prev) => {
      ...
      return {
        ...
        resultados: {
          ...prev.resultados,
          [id]: { valor, origem, ...(prevField?.ocrConfianca !== undefined && origem !== 'MANUAL' && { ocrConfianca: prevField.ocrConfianca }) },
        },
      };
    });
  }
  ...
  function setNumerico(id: 'ph' | 'densidade', raw: string) {
    ...
    setForm((prev) => {
      ...
      return {
        ...
        resultados: {
          ...prev.resultados,
          [id]: { valor: num, origem, ...(prevField?.ocrConfianca !== undefined && origem !== 'MANUAL' && { ocrConfianca: prevField.ocrConfianca }) },
        },
      };
    });
  }
  ...
  function applyOcr(result: OcrResult) {
    setForm((prev) => {
      ...
      for (const id of analytesSugeriveis()) {
        const sug = result[id];
        if (!sug || sug.valor === null || sug.confidence < 0.60) {
          // não preenche — deixa o operador digitar manualmente
          continue;
        }
        if (id === 'ph' || id === 'densidade') {
          if (typeof sug.valor === 'number') {
            updatedResultados[id] = { valor: sug.valor, origem: 'OCR_ACEITO', ocrConfianca: sug.confidence };
          }
        } else {
          if (typeof sug.valor === 'string') {
            updatedResultados[id] = { valor: sug.valor as UroValorCategorico, origem: 'OCR_ACEITO', ocrConfianca: sug.confidence };
          }
        }
      }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (naoConforme && !result.data.acaoCorretiva?.trim()) {
      setErrors({ acaoCorretiva: 'Ação corretiva é obrigatória em caso de não conformidade (RDC 978/2025 Art. 128).' });
  ...
  function renderAnalyte(id: UroAnalitoId) {
    ...
    if (id === 'ph' || id === 'densidade') {
      ...
      const isOk  = valor === null ? null : (valor as number) >= range.min && (valor as number) <= range.max;
        ...
        <div className="grid grid-cols-[1fr_120px_90px] gap-3 items-start">
          <div className="pt-2.5">
            <p className="text-sm font-medium text-slate-700 dark:text-white/75">
              {label} {neverOcr && <span className="ml-1 text-[9px] font-normal text-slate-400 dark:text-white/25">(sempre manual)</span>}
            </p>
          ...
          <div>
            <input
              ...
              className={[
                isOk === null ? INPUT : (isOk ? INPUT.replace('border-slate-200 dark:border-white/[0.09]', 'border-emerald-400/50') : INPUT.replace('border-slate-200 dark:border-white/[0.09]', 'border-red-400/60')),
              ].join(' ')}
            />
          </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-7" noValidate>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.07]">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-slate-400 dark:text-white/30 mt-0.5">Data e hora registradas automaticamente ao salvar</p>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <Label htmlFor="cargo" required>Cargo profissional</Label>
              <select id="cargo" value={form.cargo ?? ''} onChange={(e) => setField('cargo', e.target.value as UroanaliseFormData['cargo'])} className={errors.cargo ? INPUT_ERR : INPUT}>
            ...
            <div>
              ...
              <input id="operatorDocument" type="text" placeholder="ex: CRBM-MG 12345" value={form.operatorDocument ?? ''} onChange={(e) => setField('operatorDocument', e.target.value)} className={errors.operatorDocument ? INPUT_ERR : INPUT}/>
      ...
      <section>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <select id="frequencia" value={form.frequencia ?? ''} onChange={(e) => setField('frequencia', e.target.value as UroanaliseFormData['frequencia'])} className={errors.frequencia ? INPUT_ERR : INPUT}>
      ...
      <section>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <Label htmlFor="loteTira" required>Lote das tiras</Label>
              <input id="loteTira" type="text" placeholder="ex: 0034-24" value={form.loteTira ?? ''} onChange={(e) => setField('loteTira', e.target.value)} className={errors.loteTira ? INPUT_ERR : INPUT}/>
              <FieldError msg={errors.loteTira}/>
            </div>
            <div>
              <Label htmlFor="tiraMarca">Marca</Label>
              <input id="tiraMarca" type="text" placeholder="ex: Combur-10, Multistix-10SG" value={form.tiraMarca ?? ''} onChange={(e) => setField('tiraMarca', e.target.value)} className={INPUT}/>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <Label htmlFor="fabricanteTira">Fabricante</Label>
              <input id="fabricanteTira" type="text" placeholder="ex: Roche, Siemens, Bioeasy" value={form.fabricanteTira ?? ''} onChange={(e) => setField('fabricanteTira', e.target.value)} className={INPUT}/>
            </div>
            <div>
              <Label htmlFor="validadeTira">Validade da tira</Label>
              <input id="validadeTira" type="date" value={form.validadeTira ?? ''} onChange={(e) => setField('validadeTira', e.target.value)} className={errors.validadeTira ? INPUT_ERR : INPUT}/>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input id="loteControle" type="text" placeholder="ex: L2024-021" value={form.loteControle ?? ''} onChange={(e) => setField('loteControle', e.target.value)} className={errors.loteControle ? INPUT_ERR : INPUT}/>
              <FieldError msg={errors.loteControle}/>
            </div>
            <div>
              <Label htmlFor="fabricanteControle" required>Fabricante</Label>
              <input id="fabricanteControle" type="text" placeholder="ex: Bio-Rad Liquichek" value={form.fabricanteControle ?? ''} onChange={(e) => setField('fabricanteControle', e.target.value)} className={errors.fabricanteControle ? INPUT_ERR : INPUT}/>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input id="aberturaControle" type="date" value={form.aberturaControle ?? ''} onChange={(e) => setField('aberturaControle', e.target.value)} className={errors.aberturaControle ? INPUT_ERR : INPUT}/>
              <FieldError msg={errors.aberturaControle}/>
            </div>
            <div>
              <Label htmlFor="validadeControle" required>Validade</Label>
              <input id="validadeControle" type="date" value={form.validadeControle ?? ''} onChange={(e) => setField('validadeControle', e.target.value)} className={errors.validadeControle ? INPUT_ERR : INPUT}/>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input id="temperaturaAmbiente" type="number" step="0.1" value={form.temperaturaAmbiente ?? ''} onChange={(e) => setField('temperaturaAmbiente', e.target.value === '' ? undefined : Number(e.target.value))} className={INPUT}/>
          </div>
          <div>
            <Label htmlFor="umidadeAmbiente">Umidade (%)</Label>
            <input id="umidadeAmbiente" type="number" step="1" min="0" max="100" value={form.umidadeAmbiente ?? ''} onChange={(e) => setField('umidadeAmbiente', e.target.value === '' ? undefined : Number(e.target.value))} className={INPUT}/>
          </div>
        </div>
      </section>
      ...
      {ocrEnabled && (
        <section>
          ...
          <OcrCameraButton labId={labId} onDone={applyOcr} loading={ocrLoading} setLoading={setOcrLoading}/>
        </section>
      )}
      ...
      {(naoConforme || form.acaoCorretiva) && (
        <section>
          ...
          <div>
            ...
            <textarea id="acaoCorretiva" rows={3} value={form.acaoCorretiva ?? ''} onChange={(e) => setField('acaoCorretiva', e.target.value)} placeholder="Ex: Repetida a corrida com tira nova do mesmo lote; resultado dentro do critério na segunda leitura…"
      ...
      {naoConforme && (
        <section className="rounded-2xl border border-amber-400/25 bg-amber-500/[0.04] p-5">
          <SectionTitle hint="RDC 67/2009 · RDC 551/2021">Notificação sanitária — NOTIVISA</SectionTitle>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select id="notivisaTipo" value={form.notivisaTipo ?? ''} onChange={(e) => setField('notivisaTipo', e.target.value === '' ? undefined : (e.target.value as UroanaliseFormData['notivisaTipo']))} className={errors.notivisaTipo ? INPUT_ERR : INPUT}>
              ...
              <div>
                ...
                <select id="notivisaStatus" value={form.notivisaStatus ?? ''} onChange={(e) => setField('notivisaStatus', e.target.value === '' ? undefined : (e.target.value as UroanaliseFormData['notivisaStatus']))} className={errors.notivisaStatus ? INPUT_ERR : INPUT}>
                  <option value="">— selecione —</option>
            ...
            {form.notivisaStatus === 'notificado' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input id="notivisaProtocolo" type="text" placeholder="ex: 2026.123456" value={form.notivisaProtocolo ?? ''} onChange={(e) => setField('notivisaProtocolo', e.target.value)} className={errors.notivisaProtocolo ? INPUT_ERR : INPUT}/>
                  <FieldError msg={errors.notivisaProtocolo}/>
                </div>
                <div>
                  <Label htmlFor="notivisaDataEnvio" required>Data de envio</Label>
                  <input id="notivisaDataEnvio" type="date" value={form.notivisaDataEnvio ?? ''} onChange={(e) => setField('notivisaDataEnvio', e.target.value)} className={errors.notivisaDataEnvio ? INPUT_ERR : INPUT}/>
            ...
            {form.notivisaStatus === 'dispensado' && (
              <div>
                ...
                <textarea id="notivisaJustificativa" rows={3} placeholder="Causa raiz operacional…" value={form.notivisaJustificativa ?? ''} onChange={(e) => setField('notivisaJustificativa', e.target.value)} className={[errors.notivisaJustificativa ? INPUT_ERR : INPUT, 'resize-y min-h-[72px]'].join(' ')}/>
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-slate-200/60 dark:border-white/[0.06]">
        {onCancel && (
          <button type="button" onClick={onCancel} disabled={isSaving} className="px-4 h-10 rounded-xl text-sm font-medium text-slate-500 dark:text-white/50 hover:text-slate-800 dark:hover:text-white/80 hover:bg-slate-100 dark:hover:bg-white/[0.05] transition-all disabled:opacity-40">
            Cancelar
          </button>
        )}
        <button type="submit" disabled={isSaving} className={[
          ...
          naoConforme ? 'bg-amber-500 hover:bg-amber-600 shadow-amber-500/20' : 'bg-amber-600 hover:bg-amber-500 shadow-amber-500/20',
