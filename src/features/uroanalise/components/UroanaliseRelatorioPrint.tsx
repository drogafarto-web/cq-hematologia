...
function formatNotivisaCell(run: UroanaliseRun): string {
  ...
  if (run.notivisaStatus === 'notificado') return `✓ Protocolo ${run.notivisaProtocolo ?? '—'} (${fmtDate(run.notivisaDataEnvio)})`;
  if (run.notivisaStatus === 'dispensado') return `Dispensado — ${run.notivisaJustificativa ?? 'sem justificativa'}`;
...
const STATUS_LABELS: Record<UroLotStatus, string> = {
  sem_dados: 'Sem dados', valido: 'Válido', atencao: 'Atenção', reprovado: 'Reprovado',
};

const CARGO_LABELS: Record<string, string> = {
  biomedico: 'Biomédico(a)', tecnico: 'Técnico(a) de Laboratório', farmaceutico: 'Farmacêutico(a)',
};
...
export function UroanaliseRelatorioPrint({ lot, runs, lotStatus, onClose }: Props) {
  ...
  const summary = useMemo(() => {
    ...
    const notivisaPendentes = runs.filter((r) => r.conformidade === 'R' && (!r.notivisaStatus || r.notivisaStatus === 'pendente')).length;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/80 dark:bg-black/85 backdrop-blur-sm overflow-y-auto" role="dialog" aria-modal="true">
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-3 bg-slate-100 dark:bg-[#0F1318] border-b border-slate-200 dark:border-white/[0.08] print:hidden">
        <div>
          <p className="text-sm font-semibold text-slate-900 dark:text-white">Relatório de CIQ — Uroanálise</p>
          <p className="text-xs text-slate-400 dark:text-white/30">{audCode}</p>
        </div>
        <div className="flex items-center gap-2">
          <button type="button" onClick={handlePrint} className="inline-flex items-center gap-2 h-9 px-4 rounded-lg text-sm font-semibold bg-amber-600 hover:bg-amber-500 text-white transition-colors">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" aria-hidden><path d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2"/><path d="M6 14h12v8H6z"/></svg>
            Imprimir / Salvar PDF
          </button>
          <button type="button" onClick={onClose} className="h-9 px-3 rounded-lg text-xs font-medium text-slate-500 dark:text-white/55 hover:text-slate-800 dark:hover:text-white/85 hover:bg-slate-200 dark:hover:bg-white/[0.05] transition-all">
            Fechar
          </button>
        </div>
      </div>
          ...
          <article className="p-10 print:p-0 text-[11px] leading-relaxed">
            ...
            <header className="pb-4 border-b-2 border-slate-900 mb-4">
              <div className="flex items-start justify-between gap-6">
                <div>
                  <p className="text-[9px] uppercase tracking-widest text-slate-500 mb-1">CIQ — Uroanálise por tiras reagentes</p>
                  <h1 className="text-xl font-bold tracking-tight">{activeLab?.name ?? 'Laboratório'}</h1>
                  {activeLab?.legalName && <p className="text-[10px] text-slate-500 mt-0.5">{activeLab.legalName}</p>}
                  {activeLab?.cnpj && <p className="text-[9px] text-slate-500 mt-0.5">CNPJ {activeLab.cnpj}</p>}
                </div>
            ...
            <section className="mb-5 avoid-break">
              <h2 className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 border-b border-slate-200 pb-1">Lote de Controle</h2>
              <div className="grid grid-cols-4 gap-x-6 gap-y-2 text-[10.5px]">
                ...
                <Field label="Decisão formal" value={lot.uroDecision ? (lot.uroDecision === 'A' ? 'Aceitável' : lot.uroDecision === 'NA' ? 'Não aceitável' : 'Rejeitado') : '— pendente'}/>
              </div>
            </section>

            {/* Sumário */}
            <section className="mb-5 avoid-break">
              <h2 className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 border-b border-slate-200 pb-1">Sumário</h2>
            ...
            <section className="mb-6 avoid-break">
              <h2 className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 border-b border-slate-200 pb-1">Conformidade por analito</h2>
                ...
                <tbody>
                  {hitRate.map((row, idx) => (
                    <tr key={row.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                      ...
                      <td className={`px-2 py-1 text-right tabular-nums font-semibold ${row.taxa === null ? '' : row.taxa >= 95 ? 'text-emerald-700' : row.taxa >= 85 ? 'text-amber-700' : 'text-red-700'}`}>
            ...
            <section className="mb-6">
              <h2 className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 border-b border-slate-200 pb-1">Corridas cronológicas</h2>
              <table className="w-full border-collapse text-[9px]">
                <thead>
                  <tr className="bg-slate-100 border-y border-slate-300">
                    ...
                    {URO_ANALITOS.map((id) => (
                      <th key={id} className="text-center px-1 py-1 font-semibold text-[8px]">{URO_ANALITO_LABELS[id].slice(0, 4)}</th>
                    ))}
                ...
                <tbody>
                  {sortedRuns.map((r, idx) => {
                    ...
                    return (
                      <tr key={r.id} className={[idx % 2 === 0 ? 'bg-white' : 'bg-slate-50', isNC ? 'bg-red-50' : ''].join(' ')}>
                        ...
                        {URO_ANALITOS.map((id) => {
                          ...
                          return (
                            <td key={id} className={`px-1 py-1 text-center text-[8.5px] ${ncAnalyte ? 'text-red-700 font-semibold' : ''}`}>
                              {formatValor(id, r)}
                            </td>
                          );
                        })}
                        <td className={`px-1.5 py-1 font-semibold ${isNC ? 'text-red-700' : 'text-emerald-700'}`}>{isNC ? 'R' : 'A'}</td>
              ...
              <p className="text-[8.5px] text-slate-500 mt-1.5">
                Linhas em vermelho indicam não conformidade. Valores em vermelho extrapolam o critério de aceitabilidade do nível. Abreviação: Uro=Urobilinogênio · Glic=Glicose · Cet=Cetonas · Bili=Bilirrubina · Prot=Proteína · Nitr=Nitrito · pH · Sang=Sangue · Dens=Densidade · Leuc=Leucócitos.
              </p>
            </section>
            ...
            {sortedRuns.some((r) => r.conformidade === 'R' && r.acaoCorretiva?.trim()) && (
              <section className="mb-5 avoid-break">
                <h2 className="text-[10px] uppercase tracking-widest text-slate-500 mb-2 border-b border-slate-200 pb-1">Ações corretivas (RDC 978/2025 Art. 128)</h2>
              ...
              <div className="grid grid-cols-2 gap-12">
                <div>
                  ...
                  <p className="text-[9px] text-slate-500">
                    {lot.decisionBy && user?.uid === lot.decisionBy ? user?.displayName ?? user?.email : '—'}
                  </p>
                  {lot.decisionAt && <p className="text-[9px] text-slate-500 mt-0.5">Decidido em {fmtTs(lot.decisionAt)}</p>}
                </div>
                <div>
                  <div className="border-b border-slate-900 h-12"/>
                  <p className="text-[9.5px] mt-1 text-slate-600">Operador / Biomédico</p>
                  <p className="text-[9px] text-slate-500">{sortedRuns[sortedRuns.length - 1]?.operatorName ?? '—'}</p>
                  <p className="text-[9px] text-slate-500">
            ...
            <footer className="mt-10 pt-4 border-t border-slate-300 text-[8.5px] text-slate-500 leading-relaxed">
              <p>
                Documento emitido eletronicamente em conformidade com <strong>RDC 302/2005</strong>, <strong>RDC 978/2025</strong> (Art. 128 — ação corretiva em toda não conformidade), e <strong>RDC 67/2009 · 551/2021</strong> (Tecnovigilância NOTIVISA). Referências técnicas: CLSI GP16-A3 · European Urinalysis Guidelines (EUG). Cada corrida possui assinatura lógica SHA-256 vinculando operador, lote e resultados — verificável via QR Code.
              </p>
              <p className="mt-1.5 font-mono">{audCode} · {activeLab?.id ?? ''} · {lot.id}</p>
            </footer>
