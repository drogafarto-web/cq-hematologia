...
const EMPTY_ACAO: Acao5W2H = { what: '', why: '', where: '', when: '', who: '', how: '', howMuch: '' };
...
const FIELD_LABELS: { key: keyof Acao5W2H; label: string; placeholder: string }[] = [
  ...
  { key: 'howMuch', label: 'Quanto custa? (How much)', placeholder: 'Recursos ou investimento necessário' },
];
...
export function FiveW2HForm({ initialData, onSave, onCancel, saving }: Props) {
  ...
  const isValid = acoes.some((a) => a.what.trim().length >= 5 && a.who.trim().length >= 2 && a.when.trim().length >= 2);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="flex items-center gap-2 pb-3 border-b border-white/10">
        <span className="w-7 h-7 rounded-lg bg-violet-500/20 flex items-center justify-center text-sm font-bold text-violet-400">W</span>
      ...
      {acoes.map((acao, idx) => (
        <div key={idx} className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-3">
          <div className="flex items-center justify-between">
            ...
            {acoes.length > 1 && (
              <button type="button" onClick={() => removeAcao(idx)} className="text-red-400/60 hover:text-red-400 text-xs" disabled={saving}>
                Remover
              </button>
            )}
          </div>
          {FIELD_LABELS.map(({ key, label, placeholder }) => (
            <div key={key} className="space-y-1">
              <label className="text-xs font-medium text-white/60">
                {label}
                {(key === 'what' || key === 'who' || key === 'when') && <span className="text-red-400 ml-0.5">*</span>}
              </label>
              <input
                type="text"
                value={acao[key]}
      ...
      <button type="button" onClick={addAcao} className="text-xs text-violet-400 hover:text-violet-300" disabled={saving}>
