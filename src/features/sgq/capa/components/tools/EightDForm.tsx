...
const STEPS = [
  ...
  {
    key: 'd2',
    label: 'D2 — Descrição do Problema',
    hint: 'Descreva o problema com dados objetivos (É/Não é)',
  },
  {
    key: 'd3',
    label: 'D3 — Ação de Contenção',
    hint: 'Ação imediata para conter o impacto enquanto investiga',
  },
  {
    key: 'd4',
    label: 'D4 — Causa Raiz',
    hint: 'Use Ishikawa + 5 Porquês para identificar a causa fundamental',
  },
  {
    key: 'd5',
    label: 'D5 — Ação Corretiva Permanente',
    hint: 'Solução definitiva que elimina a causa raiz',
  },
  {
    key: 'd6',
    label: 'D6 — Implementação',
    hint: 'Plano de execução da ação corretiva (quem, quando, como)',
  },
  {
    key: 'd7',
    label: 'D7 — Prevenção de Recorrência',
    hint: 'Padronização, treinamento, poka-yoke, atualização de POP',
  },
  {
    key: 'd8',
    label: 'D8 — Reconhecimento',
    hint: 'Encerramento formal e reconhecimento da equipe',
  },
] as const;
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-2">
        ...
        <input
          ...
          placeholder="Líder da investigação *"
          className={INPUT_CLS}
          disabled={saving}
        />
        <input
      ...
      <div className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-2">
        ...
        <input
          type="text"
          value={d3Acao}
          onChange={(e) => setD3Acao(e.target.value)}
          placeholder="Ação de contenção imediata"
          className={INPUT_CLS}
          disabled={saving}
        />
        <div className="grid grid-cols-2 gap-2">
          <input
            type="text"
            value={d3Resp}
            onChange={(e) => setD3Resp(e.target.value)}
          ...
          <input
            type="text"
            value={d3Prazo}
            onChange={(e) => setD3Prazo(e.target.value)}
            placeholder="Prazo"
            className={INPUT_CLS}
            disabled={saving}
          />
      ...
      <div className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-2">
        ...
        <textarea
          ...
          placeholder="Ação corretiva permanente"
          className={`${INPUT_CLS} resize-none`}
          rows={2}
          disabled={saving}
        />
      ...
      <div className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-2">
        ...
        <textarea
          ...
          placeholder="Ações de prevenção de recorrência"
          className={`${INPUT_CLS} resize-none`}
          rows={2}
          disabled={saving}
        />
      ...
      <div className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-2">
        ...
        <textarea
          ...
          placeholder="Reconhecimento da equipe e encerramento"
          className={`${INPUT_CLS} resize-none`}
          rows={2}
          disabled={saving}
        />
