...
export function ParetoForm({ initialData, onSave, onCancel, saving }: Props) {
  ...
  function updateItem(idx: number, field: 'categoria' | 'frequencia', value: any) {
    const updated = [...itens];
    updated[idx] = { ...updated[idx], [field]: field === 'frequencia' ? Math.max(0, parseInt(value) || 0) : value };
    setItens(updated);
  }
  ...
  const computed = useMemo(() => {
    ...
    return sorted.map((item) => {
      ...
      return { ...item, percentual: Math.round(percentual * 10) / 10, acumulado: Math.round(acumulado * 10) / 10 };
    });
  }, [itens]);
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <input type="date" value={periodoInicio} onChange={(e) => setPeriodoInicio(e.target.value)} className={INPUT_CLS} disabled={saving} />
        </div>
        <div className="space-y-1">
          <label className="text-xs font-medium text-white/60">Período fim</label>
          <input type="date" value={periodoFim} onChange={(e) => setPeriodoFim(e.target.value)} className={INPUT_CLS} disabled={saving} />
        </div>
      </div>

      <div className="space-y-2">
        <label className="text-sm font-medium text-white/90">Categorias e frequências <span className="text-red-400">*</span></label>
        <p className="text-[10px] text-white/30">Mínimo 2 categorias. Ex: tipos de NC, setores, causas recorrentes.</p>
        {itens.map((item, idx) => (
          <div key={idx} className="flex gap-2 items-center">
            ...
            />
            {itens.length > 1 && (
              <button type="button" onClick={() => removeItem(idx)} className="text-red-400/60 hover:text-red-400 text-xs" disabled={saving}>✕</button>
            )}
          </div>
        ))}
      ...
      {computed.length >= 2 && (
        <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-2">
          <p className="text-xs font-medium text-white/60 mb-3">Resultado (ordenado por frequência):</p>
          {computed.map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex items-center gap-2 text-xs">
                ...
                <span className="w-10 text-right tabular-nums text-white/50">{item.frequencia}</span>
                <span className="w-12 text-right tabular-nums text-white/40">{item.percentual}%</span>
                <span className={`w-14 text-right tabular-nums ${item.acumulado! >= 80 && idx === corte80 ? 'text-amber-400' : 'text-white/30'}`}>
                  {item.acumulado}%
                </span>
              </div>
            );
          ...
          {corte80 >= 0 && (
            <p className="text-[10px] text-amber-400 mt-2">
              Linha 80%: as {corte80 + 1} primeiras categorias representam {computed[corte80]?.acumulado}% das ocorrências
            </p>
          )}
        </div>
      )}
      ...
      <div className="space-y-2">
        ...
        <textarea value={conclusao} onChange={(e) => setConclusao(e.target.value)} placeholder="Com base no Pareto, quais categorias devem ser priorizadas?" className={`${INPUT_CLS} resize-none`} rows={2} disabled={saving} />
      </div>
