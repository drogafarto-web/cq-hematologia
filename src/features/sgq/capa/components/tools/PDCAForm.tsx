...
const FASE_INFO: Record<PDCAFase, { label: string; color: string; hint: string }> = {
  plan: {
    label: 'Plan (Planejar)',
    color: 'text-blue-400',
    hint: 'Identifique o problema, analise causas, defina metas e ações',
  },
  do: {
    label: 'Do (Executar)',
    color: 'text-emerald-400',
    hint: 'Implemente as ações planejadas, treine a equipe',
  },
  check: {
    label: 'Check (Verificar)',
    color: 'text-amber-400',
    hint: 'Monitore indicadores, compare resultados com metas',
  },
  act: {
    label: 'Act (Agir)',
    color: 'text-violet-400',
    hint: 'Padronize se eficaz, ou inicie novo ciclo se ineficaz',
  },
};
...
export function PDCAForm({ initialData, onSave, onCancel, saving }: Props) {
  ...
  const [fases, setFases] = useState(
    initialData?.fases ?? { plan: '', do: '', check: '', act: '' },
  );
  const [faseAtual, setFaseAtual] = useState<PDCAFase>(initialData?.faseAtual ?? 'plan');
  const [resultado, setResultado] = useState<'em-andamento' | 'eficaz' | 'novo-ciclo'>(
    initialData?.resultado ?? 'em-andamento',
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-2">
          <label className="text-sm font-medium text-white/90">
            Objetivo do ciclo <span className="text-red-400">*</span>
          </label>
          <input
            type="text"
            value={objetivo}
            onChange={(e) => setObjetivo(e.target.value)}
            placeholder="O que este ciclo pretende resolver?"
            className={INPUT_CLS}
            disabled={saving}
          />
        </div>
        <div className="space-y-2">
          ...
          <input
            type="text"
            value={indicadorSucesso}
            onChange={(e) => setIndicadorSucesso(e.target.value)}
            placeholder="Como medir se funcionou?"
            className={INPUT_CLS}
            disabled={saving}
          />
        </div>
      ...
      <div className="space-y-3">
        {(Object.keys(FASE_INFO) as PDCAFase[]).map((fase) => (
          <div
            key={fase}
            className={`p-3 rounded-xl border ${faseAtual === fase ? 'bg-white/[0.04] border-violet-500/30' : 'bg-white/[0.01] border-white/[0.06]'}`}
          >
            <div className="flex items-center justify-between mb-2">
              <span
                className={`text-xs font-semibold uppercase tracking-wider ${FASE_INFO[fase].color}`}
              >
              ...
              <button
                ...
                className={`text-[10px] px-2 py-0.5 rounded ${faseAtual === fase ? 'bg-violet-500/20 text-violet-300' : 'bg-white/5 text-white/40 hover:text-white/60'}`}
              >
