...
const BTN_DANGER =
  'px-2.5 py-1 text-[10px] font-medium text-red-300 hover:text-red-200 hover:bg-red-500/10 rounded-lg transition-colors';
const BTN_GHOST =
  'px-3 py-1.5 text-xs font-medium text-white/60 hover:text-white hover:bg-white/[0.06] rounded-xl transition-colors';
const INPUT_CLS =
  'w-full px-3.5 py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.09] text-white/90 placeholder-white/20 text-sm focus:outline-none focus:border-violet-500/50 focus:bg-white/[0.08] disabled:opacity-40 transition-all';
const SELECT_CLS =
  'px-2.5 py-1.5 rounded-lg bg-white/[0.06] border border-white/[0.09] text-xs text-white/80';
                ...
                {items.map((a) => (
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span
                          className={`inline-block w-1.5 h-1.5 rounded-full ${a.ativa ? 'bg-emerald-400' : 'bg-red-400'}`}
                        />
