...
import type { TreinamentoFilters, StatusTreinamento, TipoTreinamento } from '../types/Treinamento';
...
export function TreinamentosList() {
  ...
  const handleFilterChange = (key: keyof TreinamentoFilters, value: any) => {
    setFilters((prev) => ({
      ...prev,
      [key]: value || undefined,
    }));
            ...
            <tbody>
              {treinamentos.map((t) => (
                <tr key={t.id} className="border-b border-gray-800 hover:bg-gray-900/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-400">
                    {Object.values(t.presenca).filter((p) => p.presente).length} / {t.participantes.length}
                  </td>
