  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B0F14] text-slate-900 dark:text-white">
      <header className="h-14 bg-white dark:bg-[#0F1318] border-b border-slate-200 dark:border-white/[0.06] flex items-center gap-4 px-6 sticky top-0 z-10">
        <button type="button" onClick={goBack} className={BUTTON_GHOST} aria-label="Voltar ao contexto anterior">
          ← Voltar
        </button>
        <div className="h-5 w-px bg-slate-200 dark:bg-white/[0.08]" />
        <div>
          <div className="text-sm font-medium text-slate-900 dark:text-white/85">Pré & Pós-analítico</div>
      ...
      <main className="max-w-2xl mx-auto px-8 py-10 space-y-6">
        <p className="text-xs font-bold tracking-widest uppercase text-fuchsia-400/90">Bloco 8 · SGQ</p>
        ...
        <p className="text-sm text-slate-600 dark:text-white/50 leading-relaxed">
          Este espaço consolida o escopo pré-analítico, analítico e pós-analítico do DICQ. Os fluxos de order
          entry, triagem com valores críticos e revisão de laudos serão expandidos aqui; enquanto isso, use os
          módulos operacionais já ligados ao laboratório (CIQ, críticos, portal, exportações).
        </p>
