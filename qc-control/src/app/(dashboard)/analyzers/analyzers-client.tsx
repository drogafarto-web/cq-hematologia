...
export function AnalyzersClient({ analyzers: initial }: AnalyzersClientProps) {
  ...
  async function handleMaintenance(data: {
    ...
    try {
      ...
      setAnalyzers((prev) =>
        prev.map((a) =>
          a.id === selectedAnalyzerId ? { ...a, maintenances: [maint, ...a.maintenances] } : a,
        ),
      );
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 flex flex-col gap-6">
      ...
      {showCalibrationForm && (
        <CalibrationForm onSave={handleCalibrate} onClose={() => setShowCalibrationForm(false)} />
      )}

      {showMaintenanceForm && (
        <MaintenanceForm onSave={handleMaintenance} onClose={() => setShowMaintenanceForm(false)} />
      )}

      <Modal open={showCreateForm} onClose={() => setShowCreateForm(false)} title="Add Analyzer">
