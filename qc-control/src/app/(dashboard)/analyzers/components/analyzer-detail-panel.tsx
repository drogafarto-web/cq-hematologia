...
export function AnalyzerDetailPanel({
  ...
  const footer = (
    <>
      ...
      <Button variant="danger" onClick={() => onArchive(analyzer.id)}>Archive Analyzer</Button>
    </>
  )
  ...
  return (
    <Panel open={true} onClose={onClose} title={`Analyzer ${analyzer.analyzerId}`} footer={footer}>
      <div className="flex flex-col gap-6">
        {/* Identification */}
        <section>
          <h3 className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-3">Identification</h3>
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <div className="mt-0.5"><Pill variant={cfg.variant} size="sm">{cfg.label}</Pill></div>
              </div>
            </div>
            {editing ? (
              <>
                ...
                <Input label="Install Date" type="date" value={form.installDate} onChange={e => setForm(p => ({ ...p, installDate: e.target.value }))} />
              </>
            ) : (
              <>
                <div className="grid grid-cols-2 gap-3">
            ...
            {!editing && (
              <Button variant="text" size="sm" onClick={() => setEditing(true)}>Edit Fields</Button>
            )}
          </div>
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant">Calibration History</h3>
            <Button size="sm" onClick={onOpenCalibration}>Record New Calibration</Button>
          </div>
          {isCalOverdue && (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-surface-variant">
                    <th className="text-xs font-semibold text-left px-3 py-2 text-on-surface-variant">Date</th>
                    <th className="text-xs font-semibold text-left px-3 py-2 text-on-surface-variant">Cert#</th>
                    <th className="text-xs font-semibold text-left px-3 py-2 text-on-surface-variant">Next Due</th>
                    <th className="text-xs font-semibold text-left px-3 py-2 text-on-surface-variant">By</th>
                  </tr>
                </thead>
                <tbody>
                  {analyzer.calibrations.map(cal => (
                    <tr key={cal.id} className="border-b border-border">
                      ...
                      <td className={cn('px-3 py-2', daysUntil(cal.nextDueAt) < 0 && 'text-error font-semibold')}>
                        {fmt(cal.nextDueAt)}
                      </td>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant">Maintenance History</h3>
            <Button size="sm" onClick={onOpenMaintenance}>Log Maintenance</Button>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-surface-variant">
                    <th className="text-xs font-semibold text-left px-3 py-2 text-on-surface-variant">Date</th>
                    <th className="text-xs font-semibold text-left px-3 py-2 text-on-surface-variant">Type</th>
                    <th className="text-xs font-semibold text-left px-3 py-2 text-on-surface-variant">Description</th>
                    <th className="text-xs font-semibold text-left px-3 py-2 text-on-surface-variant">Outcome</th>
                    <th className="text-xs font-semibold text-left px-3 py-2 text-on-surface-variant">Technician</th>
                  </tr>
                </thead>
                <tbody>
                  {analyzer.maintenances.map(m => (
                    <tr key={m.id} className="border-b border-border">
                      ...
                      <td className="px-3 py-2 max-w-[140px] truncate" title={m.description}>{m.description}</td>
                      <td className="px-3 py-2">
                        <Pill variant={outcomeVariant[m.outcome] ?? 'neutral'} size="sm">{m.outcome.replace('_', ' ')}</Pill>
                      </td>
        ...
        {/* Traceability */}
        <section>
          <h3 className="text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-3">Traceability</h3>
          <div className="flex gap-6">
            ...
            <div>
              <span className={cn('text-2xl font-semibold', analyzer.openCaCount > 0 ? 'text-error' : 'text-on-surface')}>
