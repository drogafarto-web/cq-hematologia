...
function RulePill({
  ...
  return (
    <span className={`inline-flex px-2 py-0.5 rounded text-xs font-semibold ${bg}`}>{rule}</span>
  );
}
    ...
    return (
      <div className="flex flex-col gap-3">
        ...
        <p className="text-sm text-outline">
          No runs recorded yet. Select a lot and add a value above.
        </p>
      </div>
    );
  }
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <RulePill
                      rule={run.ruleViolated}
                      isReject={run.isReject}
                      isWarning={run.isWarning}
                    />
                  </td>
                  <td className="px-4 py-3 text-xs text-outline max-w-[160px] truncate">
                    {run.justification || '—'}
                  </td>
