...
function Field({ label, value, mono }: { label: string; value: string | null | undefined; mono?: boolean }) {
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <span className={`text-sm ${mono ? 'font-mono' : ''}`}>{value || <span className="text-outline">—</span>}</span>
    </div>
  )
}

export function CADetailPanel({ ca, users, onClose, onUpdate, onStatusChange }: CADetailPanelProps) {
      ...
      const res = await fetch(`/api/corrective-actions/${ca.id}`, {
        ...
        body: JSON.stringify({
          ...
          verificationAt: verificationAt
            ? new Date(verificationAt).toISOString()
            : undefined,
        }),
      })
      const json = await res.json()
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-xl bg-white shadow-lg flex flex-col">
        ...
        <div className="px-6 py-3 border-b border-border flex items-center justify-between shrink-0">
          <Pill variant={config.variant} size="sm">{config.label}</Pill>
          <div className="flex gap-2">
            {isOpen && (
              <Button size="sm" onClick={() => handleTransition('IN_PROGRESS')} loading={transitioning}>
                Start Investigation
              </Button>
            )}
            {isInProgress && (
              <Button size="sm" onClick={() => handleTransition('UNDER_VERIFICATION')} loading={transitioning}>
          ...
          {activeSection === 'identification' && (
            <div className="flex flex-col gap-4">
              <SectionGrid>
                ...
                <Field
                  label="Date Opened"
                  value={openedDate.toLocaleDateString('pt-BR')}
                />
