...
const mockPncqData: Record<string, { targetMean: number; sd: number; minAcceptance: number; maxAcceptance: number }> = {
...
export async function POST(req: Request) {
  ...
  if (!analyte) return NextResponse.json({ success: false, error: { code: 'VALIDATION', message: 'analyte is required' } }, { status: 400 })

  const data = mockPncqData[analyte]
  if (!data) return NextResponse.json({ success: false, error: { code: 'NOT_FOUND', message: `No PNCQ data for analyte: ${analyte}` } }, { status: 404 })
