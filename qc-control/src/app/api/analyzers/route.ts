...
export async function POST(req: Request) {
  ...
  const existing = await prisma.analyzer.findUnique({
    where: { analyzerId: parsed.data.analyzerId },
  });
  if (existing)
    return NextResponse.json(
      { success: false, error: { code: 'DUPLICATE', message: 'Analyzer ID already exists' } },
      { status: 409 },
    );
