...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (parsed.data.supportingEvidence !== undefined) data.supportingEvidence = parsed.data.supportingEvidence
  if (parsed.data.actionTaken !== undefined) data.actionTaken = parsed.data.actionTaken
  if (parsed.data.preventiveMeasure !== undefined) data.preventiveMeasure = parsed.data.preventiveMeasure
  if (parsed.data.effectivenessCheck !== undefined) data.effectivenessCheck = parsed.data.effectivenessCheck
  if (parsed.data.verifiedById !== undefined) data.verifiedById = parsed.data.verifiedById
  if (parsed.data.verificationAt !== undefined) data.verificationAt = new Date(parsed.data.verificationAt)
