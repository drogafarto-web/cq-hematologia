...
export const syncComplianceScorecard = functions.pubsub
  .schedule('0 13 * * FRI') // 09:00 BRT = 13:00 UTC
  ...
  .onRun(async (context) => {
    ...
    const auditSnapshot = await db.collectionGroup('auditLogs').where('ts', '>=', yesterday).get();
