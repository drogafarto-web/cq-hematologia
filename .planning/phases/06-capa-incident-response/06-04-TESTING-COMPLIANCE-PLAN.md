...
- Audit chain (HMAC-SHA256) integrity preserved throughout lifecycle
  ...
  <verify>
    <automated>npm test -- __tests__/capa/integration.test.ts 2>&1 | grep -E "1 passed|Tests:.*passed"</automated>
  </verify>
...
**Total: 8 tests validating decision matrix**
  ...
  <verify>
    <automated>npm test -- __tests__/incident-response/severity-matrix.test.ts 2>&1 | grep -E "8 passed|Tests:.*passed"</automated>
  </verify>
...
**Total: 6 tests, plus automated violations check via Axe**
  ...
  <verify>
    <automated>npm test -- __tests__/accessibility/wcag-aa-audit.test.ts 2>&1 | grep -E "passed|violations"</automated>
  </verify>
...
- Auditor (if applicable): _______________ Date: _______
