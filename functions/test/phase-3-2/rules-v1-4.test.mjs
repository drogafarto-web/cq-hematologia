...
const rulesDefinition = [
  ...
  },
  {
    ...
    collections: ['notivisa-outbox/events'],
    expectedRules: {
  ...
  },
  {
    ...
    collections: ['criticos-escalacoes/escalacoes'],
    expectedRules: {
  ...
  },
  {
    ...
    collections: ['imuno-ias-dev/images'],
    expectedRules: {
  ...
  },
  {
    ...
    collections: ['laudos-draft/rascunhos'],
    expectedRules: {
...
test('Portal Rules — Patient Portal Configuration & Laudo Read', async (t) => {
  ...
  await t.test(`${rule.name} — Rule definitions exist`, () => {
    ...
    assert.ok(rule.expectedRules['portal-configuracao'], 'portal-configuracao rule defined');
    assert.ok(rule.expectedRules['laudos (portal read)'], 'laudos portal read rule defined');
  });

  await t.test(`${rule.name} — Patient can read published laudo`, () => {
    const ruleSpec = rule.expectedRules['laudos (portal read)'];
    assert.ok(ruleSpec.read.includes('patient'), 'patient read access defined');
    assert.ok(ruleSpec.read.includes('own laudo only'), 'patient owns laudo restriction present');
  });

  await t.test(`${rule.name} — Admin/RT can update portal config`, () => {
    const ruleSpec = rule.expectedRules['portal-configuracao'];
    assert.ok(ruleSpec.update.includes('admin/RT'), 'admin/RT update access defined');
    assert.ok(ruleSpec.update.includes('updatedBy'), 'updatedBy validation present');
  });
});
...
test('NOTIVISA Outbox Rules — Regulatory Event Queue (RDC 978 Art. 6º)', async (t) => {
  ...
  await t.test(`${rule.name} — Rule definitions exist`, () => {
    ...
    assert.ok(rule.expectedRules['notivisa-outbox/events'], 'notivisa-outbox/events rule defined');
  });
  ...
  await t.test(`${rule.name} — RT can create NOTIVISA event with validation`, () => {
    ...
    assert.ok(ruleSpec.create.includes('admin/RT'), 'admin/RT create access defined');
    assert.ok(ruleSpec.create.includes('validateNotivisaPayload'), 'payload validation required');
  });

  await t.test(`${rule.name} — Server can read + update status`, () => {
    const ruleSpec = rule.expectedRules['notivisa-outbox/events'];
    assert.ok(ruleSpec.read.includes('server'), 'server read access defined');
    assert.ok(ruleSpec.update.includes('server'), 'server update access defined');
  });

  await t.test(`${rule.name} — Invalid payload rejected`, () => {
    const ruleSpec = rule.expectedRules['notivisa-outbox/events'];
    assert.ok(ruleSpec.create.includes('validateNotivisaPayload'), 'payload validation enforced');
  });
});
...
test('Critical Escalations Rules — Escalation Tracking', async (t) => {
  ...
  await t.test(`${rule.name} — Member can read escalations`, () => {
    const ruleSpec = rule.expectedRules['criticos-escalacoes/escalacoes'];
    assert.ok(ruleSpec.read.includes('lab member'), 'lab member read access defined');
  });

  await t.test(`${rule.name} — RT can update resolution with validation`, () => {
    const ruleSpec = rule.expectedRules['criticos-escalacoes/escalacoes'];
    assert.ok(ruleSpec.update.includes('admin/RT'), 'admin/RT update access defined');
    assert.ok(ruleSpec.update.includes('resolved_at'), 'resolved_at validation present');
  });
});
...
test('IA Strip Dev Rules — Training Dataset Access Control', async (t) => {
  ...
  await t.test(`${rule.name} — Rule definitions exist`, () => {
    assert.ok(rule.collections.length === 1, 'has 1 collection');
    assert.ok(rule.expectedRules['imuno-ias-dev/images'], 'imuno-ias-dev/images rule defined');
  });

  await t.test(`${rule.name} — Server only access enforced`, () => {
    const ruleSpec = rule.expectedRules['imuno-ias-dev/images'];
    assert.ok(ruleSpec.read.includes('server'), 'server read access defined');
    assert.ok(ruleSpec.write.includes('server'), 'server write access defined');
  });
  ...
  await t.test(`${rule.name} — Non-server blocked`, () => {
    ...
    assert.ok(ruleSpec.delete.includes('forbidden'), 'delete forbidden');
  });
});
...
test('Laudo Draft Rules — Pessimistic Locking', async (t) => {
  ...
  await t.test(`${rule.name} — Rule definitions exist`, () => {
    assert.ok(rule.collections.length === 1, 'has 1 collection');
    assert.ok(rule.expectedRules['laudos-draft/rascunhos'], 'laudos-draft/rascunhos rule defined');
  });

  await t.test(`${rule.name} — RT can acquire lock`, () => {
    const ruleSpec = rule.expectedRules['laudos-draft/rascunhos'];
    assert.ok(ruleSpec.create.includes('admin/RT'), 'admin/RT create access defined');
    assert.ok(ruleSpec.create.includes('validateDraftLock'), 'lock validation required');
  });

  await t.test(`${rule.name} — Conflict when locked by other`, () => {
    const ruleSpec = rule.expectedRules['laudos-draft/rascunhos'];
    assert.ok(ruleSpec.update.includes('validateDraftLock'), 'lock validation on update');
  });
...
test('Helper Functions — v1.4 Extensions', async (t) => {
  await t.test('All required helper functions are defined', () => {
    for (const helper of helperFunctions) {
      assert.ok(helper.name, `${helper.name} defined with purpose: ${helper.purpose}`);
    }
  });

  await t.test('isServer() identifies Cloud Function requests', () => {
    const helper = helperFunctions[0];
    assert.ok(helper.expectedLogic.includes('server'), 'server token detection logic present');
  });

  await t.test('isPatient() validates patient role', () => {
    const helper = helperFunctions[1];
    assert.ok(helper.expectedLogic.includes('patient'), 'patient role check present');
    assert.ok(
  ...
  await t.test('isAdminOrRT() covers admin + RT roles', () => {
    ...
    assert.ok(helper.expectedLogic.includes('admin'), 'admin role included');
    assert.ok(helper.expectedLogic.includes('rt'), 'rt role included');
  });

  await t.test('validateNotivisaPayload() validates RDC 978 structure', () => {
    const helper = helperFunctions[3];
    assert.ok(helper.expectedLogic.includes('laudo_id'), 'laudo_id check present');
    assert.ok(helper.expectedLogic.includes('patient_cpf'), 'patient_cpf check present');
    assert.ok(helper.expectedLogic.includes('payload'), 'payload check present');
  });

  await t.test('validateDraftLock() enforces pessimistic locking', () => {
    const helper = helperFunctions[4];
    assert.ok(helper.expectedLogic.includes('locked_until_ts'), 'lock timestamp check present');
    assert.ok(helper.expectedLogic.includes('locked_by'), 'lock owner check present');
  });
});
...
test('Security Posture — v1.4 Extensions', async (t) => {
  ...
  await t.test('Patient data isolation enforced', () => {
    ...
    assert.ok(laudoSpec.read.includes('own laudo only'), 'patient owns laudo restriction');
  });
  ...
  await t.test('Server-only collections properly restricted', () => {
    ...
    assert.ok(iaSpec.read.includes('server'), 'server-only access');
    assert.ok(!iaSpec.read.includes('patient'), 'patient denied access');
  });
...
test('Backward Compatibility — Existing Helpers', async (t) => {
  ...
  await t.test('All existing helpers still available', () => {
    assert.ok(existingHelpers.length === 7, 'expected 7 existing helpers');
  });
  ...
  await t.test('No modifications to existing rules expected', () => {
    ...
    assert.ok(addedRulesCount === rulesDefinition.length, 'only 5 new rules added');
  });
});
