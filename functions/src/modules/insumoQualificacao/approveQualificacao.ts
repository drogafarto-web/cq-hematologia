...
export const approveQualificacao = onCall<unknown, Promise<ApproveResult>>({}, async (request) => {
  ...
  if (q.qualificacaoMode === 'caracterizacao-rt') {
    throw new HttpsError('failed-precondition', 'caracterizacao-rt-not-supported-in-pr1');
  }
  ...
  if (q.qualificacaoMode === 'corrida-validacao') {
    ...
    const runsCheck = await Promise.all(
      evidenciaRunIds.map(async (runId) => {
        const cg = await db.collectionGroup('runs').where('id', '==', runId).limit(1).get();
    ...
    for (const r of runsCheck) {
      ...
      const ins = (snap['insumosSnapshot'] ?? {}) as Record<string, { id?: string } | undefined>;
