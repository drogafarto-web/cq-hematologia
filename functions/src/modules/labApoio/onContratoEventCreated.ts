/**
 * onContratoEventCreated — Firestore trigger (v2)
 *
 * Triggered when a new audit event is written to `/labs/{labId}/lab-apoio/{contratoId}/events/{eventId}`.
 *
 * Computes `chainHash = SHA-256(prevChainHash || canonicalEventPayload)` and
 * persists back to the event doc. Ensures tamper-evidence via RN-LABAPOIO-06
 * (continuidade de chainHash no audit trail).
 *
 * Mirrors onTurnoEventCreated pattern.
 */
...
export const onContratoEventCreated = onDocumentCreated(
  'labs/{labId}/lab-apoio/{contratoId}/events/{eventId}',
  async (event) => {
    const db = admin.firestore();
    const { labId, contratoId, eventId } = event.params;

    // Get the newly created event
    const eventDocRef = db.doc(`labs/${labId}/lab-apoio/${contratoId}/events/${eventId}`);
    const eventSnap = await eventDocRef.get();

    if (!eventSnap.exists) {
      console.warn(`[LABAPOIO_EVENT] Event doc does not exist: ${eventId}`);
      return;
    }
