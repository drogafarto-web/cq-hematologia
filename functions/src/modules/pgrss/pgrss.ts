 ...
 */
export const registrarGeracao = onCall({ region: 'southamerica-east1' }, async (request: any) => {
  if (!request.auth) {
 ...
 */
export const registrarColeta = onCall({ region: 'southamerica-east1' }, async (request: any) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Autenticação obrigatória');
  }
  ...
  const { labId, empresa_coletora, registroGeracaoIds, peso_total_kg, comprovante_url } =
    request.data;
  ...
  if (
    !empresa_coletora ||
    !registroGeracaoIds ||
    !Array.isArray(registroGeracaoIds) ||
    registroGeracaoIds.length === 0 ||
  ...
  try {
    ...
    await writeAuditLog({
      ...
      payload: {
        ...
        empresa_coletora,
        quantidadeRegistros: registroGeracaoIds.length,
      },
    });
 ...
 */
export const validarSegregacao = onCall({ region: 'southamerica-east1' }, async (request: any) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Autenticação obrigatória');
  }
