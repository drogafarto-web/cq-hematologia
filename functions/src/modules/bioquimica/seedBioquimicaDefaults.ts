...
async function assertActiveMemberOfLab(uid: string, labId: string): Promise<void> {
  const memberSnap = await db.collection('labs').doc(labId).collection('members').doc(uid).get();
  ...
  if (data?.active !== true) {
    throw new HttpsError('permission-denied', 'Membership inativa neste laboratório.');
  }
}
...
export const seedBioquimicaDefaults = onCall<SeedRequestPayload>(
  ...
  async (request): Promise<SeedResponse> => {
    ...
    if (forceReseed && !isSuperAdmin) {
      throw new HttpsError('permission-denied', 'forceReseed exige super-admin.');
    }
    ...
    const rootRef = db.collection('labs').doc(labId).collection('bioquimica').doc('root');
