...
export const ec_commitExecucaoRealizada = onCall<unknown, Promise<CommitRealizadaResult>>(
  {},
  async (request) => {
    ...
    const execRef = input.execucaoId ? execucoesCol.doc(input.execucaoId) : execucoesCol.doc();
    ...
    if (input.execucaoId) {
      ...
      if (status === 'realizado') {
        throw new HttpsError('failed-precondition', 'Execução já registrada como realizada.');
      }
      if (status === 'cancelado') {
        throw new HttpsError(
          'failed-precondition',
