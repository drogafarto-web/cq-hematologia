...
export const rtPresenceCheckin = onCall<unknown, Promise<CheckinResult>>({}, async (request) => {
  ...
  const colabRef = db.doc(`educacaoContinuada/${input.labId}/colaboradores/${uid}`);
  ...
  if (colab['ativo'] !== true) {
    throw new HttpsError('failed-precondition', 'Colaborador não está ativo (RN-RT-01).');
  }
