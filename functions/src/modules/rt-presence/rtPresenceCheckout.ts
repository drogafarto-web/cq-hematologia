...
export const rtPresenceCheckout = onCall<unknown, Promise<CheckoutResult>>(
  {},
  async (request) => {
    ...
    if (status['hasActiveRT'] !== true) {
      throw new HttpsError(
        'failed-precondition',
        'Nenhuma presença de RT ativa para encerramento.',
      );
    }
    ...
    if (input.sessionId !== storedSessionId) {
      throw new HttpsError(
        'permission-denied',
        'Session ID não corresponde à presença ativa.',
      );
    }
