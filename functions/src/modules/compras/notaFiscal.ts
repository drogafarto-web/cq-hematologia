...
export const criarNotaFiscal = functions.onCall(
  ...
  async (request) => {
    ...
    const { labId, numero, serie, dataEmissao, fornecedorId, itens, valorTotal } = request.data;
...
export const confirmarRecebimento = functions.onCall(
  ...
  async (request) => {
    ...
    if (!labId || !nfId) {
      throw new functions.HttpsError('invalid-argument', 'labId e nfId obrigatórios');
    }
