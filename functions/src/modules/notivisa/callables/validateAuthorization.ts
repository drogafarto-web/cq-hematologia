...
export const validateAuthorization = functions
  ...
  .onCall(
    async (
      request
    ): Promise<ValidateAuthorizationOutput | ValidateAuthorizationError> => {
      try {
        // ========== 1. Validate request ==========
        if (!request.auth) {
          return {
        ...
        const modulesClaim = (request.auth.token?.['modules'] ?? {}) as Record<
          string,
          unknown
        >;
        ...
        if (!userIsActiveMember) {
          functions.logger.warn(
            '[validateAuthorization] User not active member of lab',
            {
              uid,
              labId,
        ...
        const configSnap = await db
          .doc(`labs/${labId}/notivisa-config/portal`)
          .get();
        ...
        if (
          permissions.includes('notivisa:review') ||
          permissions.includes('notivisa:release')
        ) {
