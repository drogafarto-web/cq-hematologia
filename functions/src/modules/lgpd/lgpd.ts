 ...
 */
export const criarSolicitacao = onCall({ region: 'southamerica-east1' }, async (request: any) => {
  if (!request.auth) {
 ...
 */
export const processarExclusao = onCall({ region: 'southamerica-east1' }, async (request: any) => {
  ...
  try {
    ...
    const emailHash = crypto
      .createHash('sha256')
      .update(solicitacaoSnap.data()?.titular_email || '')
      .digest('hex');
    ...
    for (const { colecao, campo } of colecoesPorCampo) {
      ...
      for (const docSnap of snap.docs) {
        ...
        if ('usuario_email' in data)
          anonFields['usuario_email'] = `anon_${emailHash.substring(0, 8)}@anonymized.local`;
 ...
 */
export const gerarDPIA = onCall({ region: 'southamerica-east1' }, async (request: any) => {
  if (!request.auth?.token?.admin) {
    throw new HttpsError('permission-denied', 'Apenas admin podem gerar DPIA');
  }

  const { labId, titulo, descricao, dados_pessoais_processados, riscos_identificados } =
    request.data;
