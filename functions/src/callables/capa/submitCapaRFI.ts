...
const submitCapaRFIInputSchema = z.object({
  ...
  question: z.string().min(10, 'question must be at least 10 characters').max(2000, 'question max 2000 characters'),
  dueDate: z.number().int().refine((d) => d > Date.now(), 'dueDate must be in the future'),
});
