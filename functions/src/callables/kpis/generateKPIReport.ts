...
async function buildKpiPdfBuffer(params: {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc
      .fontSize(14)
      .font('Helvetica-Bold')
      .text('Relatório de indicadores (KPI)', { align: 'center' });
    doc.moveDown(0.35);
    ...
    section(
      'KPIs diários (amostra tabular)',
      rows.map(
        (r) => `${r.dataLabel} · TAT ${r.tat} · Retr. ${r.retrab} · Doc. ${r.doc} · NC ${r.nc}`,
      ),
    );
...
export const generateKPIReport = onCall<unknown, Promise<{ url: string }>>(
  ...
  async (request): Promise<{ url: string }> => {
    ...
    const metasSnap = await labRef
      .collection('kpi-metas')
      .where('ativo', '==', true)
      .limit(80)
      .get();
