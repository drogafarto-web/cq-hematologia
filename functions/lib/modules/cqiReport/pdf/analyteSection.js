...
exports.renderQuantitativeAnalyteSection = renderQuantitativeAnalyteSection;
exports.renderCategoricalRunsSection = renderCategoricalRunsSection;
const leveyJennings_1 = require("../charts/leveyJennings");
// ─── Layout constants (A4) ────────────────────────────────────────────────────
const PAGE_H = 841.89;
const MARGIN = 40;
 ...
 */
function renderQuantitativeAnalyteSection(doc, data, startY) {
    let y = ensureSpace(doc, 210, startY);
    // ── Header bar ─────────────────────────────────────────────────────────────
    doc.rect(MARGIN, y, CW, 20).fillColor('#1e3a5f').fill();
    doc
        .font(FONT_B).fontSize(8.5).fillColor('#ffffff')
        .text(`${data.analyteId}  —  ${data.name}`, MARGIN + 10, y + 6, { width: CW * 0.72 });
    doc
        .font(FONT_R).fontSize(7.5).fillColor('#93c5fd')
    ...
    if (data.last20Values.length > 0) {
        ...
        (0, leveyJennings_1.drawLeveyJenningsChart)(doc, chartInput, MARGIN, y, CW, chartH);
        y += chartH + 6;
    }
    // ── Stats table ────────────────────────────────────────────────────────────
    ...
    for (let i = 0; i < 4; i++) {
        doc
            .font(FONT_B).fontSize(6).fillColor('#475569')
    ...
    for (let i = 0; i < 4; i++) {
        doc
            .font(FONT_B).fontSize(10).fillColor('#0f172a')
    ...
    if (allViols.length > 0) {
        ...
        doc
            .font(FONT_B).fontSize(6.5).fillColor('#b91c1c')
            .text(`Violações Westgard detectadas hoje: ${allViols.join('  ·  ')}`, MARGIN + 8, y + 4, { width: CW - 16 });
        y += 18;
    }
    return y + 8;
}
 ...
 */
function renderCategoricalRunsSection(doc, runs, testTypeName, startY) {
    ...
    for (let i = 0; i < cols.length; i++) {
        doc
            .font(FONT_B).fontSize(6).fillColor('#14532d')
    ...
    for (let ri = 0; ri < runs.length; ri++) {
        ...
        const r = runs[ri];
        if (ri % 2 === 1)
            doc.rect(MARGIN, y, CW, rowH).fillColor('#f9fafb').fill();
        const cells = [
            r.runCode,
            r.testType,
        ...
        for (let i = 0; i < cells.length; i++) {
            ...
            const color = isResult
                ? (r.conforme ? '#16a34a' : '#dc2626')
                : '#111827';
            doc
                .font(isResult ? FONT_B : FONT_R).fontSize(6.5)
                .fillColor(color)
...
//# sourceMappingURL=analyteSection.js.map