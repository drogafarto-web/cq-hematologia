...
exports.drawLeveyJenningsChart = drawLeveyJenningsChart;
 ...
 */
function drawLeveyJenningsChart(doc, input, x, y, chartWidth, chartHeight) {
    ...
    doc
        ...
        .fontSize(6.5)
        .fillColor('#374151')
        .text(`${analyteId} — Lote ${lotNumber}  |  Média: ${mean.toFixed(2)}  |  SD: ${sd.toFixed(2)}  |  CV: ${cv}%`, x + 4, y + 3, { width: chartWidth - 8, ellipsis: true });
    if (values.length === 0) {
        doc
            .font('Helvetica').fontSize(8).fillColor('#9ca3af')
            .text('Sem dados para exibir', x + 4, y + titleH + 10, { width: chartWidth - 8, align: 'center' });
        doc.restore();
        return;
    }
    // ── Plot area ──────────────────────────────────────────────────────────────
    const padL = 42, padR = 10, padT = titleH + 4, padB = 24;
    const plotX = x + padL;
    const plotY = y + padT;
    ...
    const toSX = (i) => plotX + (n > 1 ? (i / (n - 1)) * plotW : plotW / 2);
    const toSY = (v) => plotY + plotH - ((Math.max(yMin, Math.min(yMax, v)) - yMin) / yRange) * plotH;
    // Plot background
    ...
    for (const rl of refLines) {
        if (sd === 0 && rl.label !== 'M')
            continue;
        const ly = toSY(rl.v);
        if (ly < plotY - 1 || ly > plotY + plotH + 1)
            continue;
        ...
        doc.moveTo(plotX, ly).lineTo(plotX + plotW, ly).stroke();
        doc.undash();
        // Y-axis label (right-aligned in left padding)
        doc
            .font('Helvetica').fontSize(5).fillColor(rl.color)
    ...
    for (let i = 0; i < n; i += step) {
        ...
        const label = timestamps[i]
            ...
            .replace(',', '');
        doc
            .font('Helvetica').fontSize(4.5).fillColor('#6b7280')
            .text(label, toSX(i) - 14, plotY + plotH + 4, { width: 30, align: 'center' });
    }
    doc.restore();
}
//# sourceMappingURL=leveyJennings.js.map