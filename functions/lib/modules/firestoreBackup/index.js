...
exports.triggerFirestoreExport_onCall = exports.scheduledFirestoreExport = void 0;
...
async function triggerFirestoreExport() {
    const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
    const outputUri = `gs://${BACKUP_BUCKET}/daily/${today}`;
    const auth = new google_auth_library_1.GoogleAuth({ scopes: 'https://www.googleapis.com/auth/datastore' });
    ...
    if (!json.name) {
        throw new Error(`[firestoreBackup] Export response missing operation name: ${JSON.stringify(json)}`);
    }
    return {
...
async function writeBackupLog(result, kind, actorUid) {
    try {
        await admin.firestore().collection('firestore-backup-logs').doc(result.triggeredAt.slice(0, 10)).set({
            kind,
...
exports.scheduledFirestoreExport = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        console.log(`[firestoreBackup][scheduled] Triggered export. ` +
 ...
 */
exports.triggerFirestoreExport_onCall = (0, https_1.onCall)({
    region: 'southamerica-east1',
    memory: '256MiB',
    timeoutSeconds: 540,
}, async (request) => {
