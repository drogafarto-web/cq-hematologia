...
exports.generateBackupPdf = generateBackupPdf;
 ...
 */
function generateBackupPdf(report) {
...
function renderCoverPage(doc, report) {
    ...
    doc
        ...
        .fontSize(14)
        .fillColor(COLOR.textPrimary)
        .text(`${formatDate(report.periodStart)}  →  ${formatDate(report.periodEnd)}`, margin, periodY + 16, { width: contentWidth, align: 'center' });
    // Stats summary
    ...
    if (report.stalenessAlerts.length > 0) {
        ...
        doc
            ...
            ? COLOR.danger
            : COLOR.accentWarm)
            .text(`⚠  ${report.stalenessAlerts.length} alerta(s) de inatividade detectado(s) — ver página de integridade`, margin + 12, alertY + 6, { width: contentWidth - 24 });
    ...
    doc
        ...
        .fontSize(8)
        .fillColor(COLOR.textMuted)
        .text(`Gerado em: ${new Date(report.generatedAt).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}  ·  HC Quality Backup Module`, margin, PAGE.height - 55, { width: contentWidth, align: 'center' });
    // Bottom accent bar
    doc
...
// ─── Module Section ───────────────────────────────────────────────────────────
function renderModuleSection(doc, section, report) {
    const { margin } = PAGE;
    const contentWidth = PAGE.width - margin * 2;
    ...
    doc
        ...
        .fontSize(9)
        .fillColor(COLOR.textMuted)
        .text(`${report.labName}  ·  ${formatDate(report.periodStart)} – ${formatDate(report.periodEnd)}`, margin, 50);
    // Summary stats
...
// ─── Data Table ───────────────────────────────────────────────────────────────
function renderDataTable(doc, columns, rows, startY, contentWidth) {
    const { margin } = PAGE;
    ...
    for (let i = 0; i < columns.length; i++) {
        doc
            ...
            .fontSize(6.5)
            .fillColor(COLOR.textMuted)
            .text(columns[i].toUpperCase(), margin + i * colWidth + 4, startY + 7, { width: colWidth - 8, ellipsis: true });
    }
    ...
    for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
        if (currentY + rowHeight > maxY) {
            ...
            for (let i = 0; i < columns.length; i++) {
                doc
                    ...
                    .fontSize(6.5)
                    .fillColor(COLOR.textMuted)
                    .text(columns[i].toUpperCase(), margin + i * colWidth + 4, currentY + 7, { width: colWidth - 8, ellipsis: true });
            }
            currentY += headerH;
        }
        ...
        for (let i = 0; i < columns.length; i++) {
            ...
            if (val === 'Conforme' || val === 'Aprovada')
                cellColor = COLOR.success;
            if (val === 'NÃO CONFORME' || val === 'Rejeitada')
                cellColor = COLOR.danger;
            if (val === 'Pendente')
                cellColor = COLOR.accentWarm;
            doc
                .font(FONT_REGULAR)
                .fontSize(6.5)
                .fillColor(cellColor)
                .text(val, margin + i * colWidth + 5, currentY + 5, { width: colWidth - 10, ellipsis: true });
        }
        currentY += rowHeight;
    }
    if (rows.length === 0) {
...
// ─── Integrity Page ───────────────────────────────────────────────────────────
function renderIntegrityPage(doc, report) {
    const { margin } = PAGE;
    const contentWidth = PAGE.width - margin * 2;
    doc
        .rect(0, 0, PAGE.width, 4)
    ...
    doc
        ...
        .fontSize(9)
        .fillColor(COLOR.textMuted)
        .text('Este documento foi gerado automaticamente pelo módulo de backup do HC Quality. ' +
        'O hash abaixo é derivado do conteúdo de todos os registros incluídos neste relatório ' +
        'e pode ser usado para verificar a integridade do arquivo em auditorias futuras.', margin, 46, { width: contentWidth });
    // Hash display
    const hashY = 100;
    doc
    ...
    doc
        ...
        .fontSize(7.5)
        .fillColor(COLOR.textMuted)
        .text('HC Quality — Backup Automático  ·  Este documento não substitui os registros primários armazenados no sistema. ' +
        'Conservar em local seguro de acordo com a política de retenção de dados da instituição (RDC 978/2025, LGPD).', margin, PAGE.height - 55, { width: contentWidth, align: 'center' });
    doc
        .rect(0, PAGE.height - 8, PAGE.width, 8)
        .fill(COLOR.accent);
}
// ─── Helpers ──────────────────────────────────────────────────────────────────
function renderStatGrid(doc, x, y, width, stats) {
...
function renderPageFooter(doc, report) {
    doc
        .font(FONT_REGULAR)
        .fontSize(7)
        .fillColor(COLOR.textMuted)
        .text(`HC Quality  ·  ${report.labName}  ·  Gerado em ${new Date(report.generatedAt).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}`, PAGE.margin, PAGE.height - 28, { width: PAGE.width - PAGE.margin * 2, align: 'center' });
    doc
        .rect(0, PAGE.height - 8, PAGE.width, 8)
        .fill(COLOR.accent);
}
