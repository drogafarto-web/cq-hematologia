...
exports.hematologiaCollector = void 0;
...
exports.hematologiaCollector = {
    ...
    async collect(db, labId, from, to) {
        ...
        for (const lotDoc of lotsSnap.docs) {
            ...
            for (const runDoc of runsSnap.docs) {
                ...
                const violations = r['westgardViolations'] ?? [];
                ...
                if (status === 'Pendente')
                    pendingCount.value++;
                if (status === 'Rejeitada' || hasRejection)
                    nonConforming++;
    ...
    async checkStaleness(db, labId, thresholdDays) {
        ...
        if (lotsSnap.empty)
            return null; // No lots = module not in use
        ...
        for (const lotDoc of allLotsSnap.docs) {
            ...
            if (!lastRunSnap.empty) {
                const ts = lastRunSnap.docs[0].data()['confirmedAt'];
...
//# sourceMappingURL=hematologiaCollector.js.map