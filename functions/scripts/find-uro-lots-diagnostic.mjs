    ...
    if (uroSnap.size > 0) {
      ...
      for (const lotDoc of uroSnap.docs) {
        ...
        runsSnap.forEach((runDoc) => {
          ...
          console.log(
            `       Run ID: ${runDoc.id} | signedBy: ${runData.createdByName || runData.createdBy || 'N/A'} | signedAt: ${runData.createdAt?.toDate ? runData.createdAt.toDate().toISOString() : runData.createdAt}`,
          );
        });
      }
    }
