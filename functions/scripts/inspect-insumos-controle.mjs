...
console.log(
  'lote     | NV | nome              | modulo      | start | valid | created | status | stats',
);
console.log(
  '---------+----+-------------------+-------------+-------+-------+---------+--------+------',
);
for (const i of items) {
  console.log(
    `${(i.lote || '?').padEnd(9)} | ${String(i.bulaLevel ?? '-').padStart(2)} | ${i.nomeComercial.padEnd(18)} | ${(i.modulo || '?').padEnd(11)} | ${i.startDate} | ${i.validade} | ${i.createdAt} | ${(i.status || '?').padEnd(6)} | ${i.hasStats ? 'yes' : 'NULL'}`,
  );
}
