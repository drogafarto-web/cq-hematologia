/**
 * provision-modules-claims.mjs
 *
 * Aplica claims de módulos a todos os usuários ativos no projeto.
 * Suporta --module para provisionar módulo específico ou todos (default).
 *
 * Uso:
 *   # Dry-run de módulo específico
 *   node scripts/provision-modules-claims.mjs --module lab-apoio
 *
 *   # Apply module específico
 *   node scripts/provision-modules-claims.mjs --module lab-apoio --apply \
 *     --reason "Provisionamento do claim Lab Apoio pós-deploy 00-03"
 *
 *   # Dry-run de todos os módulos
 *   node scripts/provision-modules-claims.mjs
 *
 * Pré-requisitos:
...
if (moduleArg) console.log(`🔧 módulo: ${moduleArg}`);
...
const runId = `provision-modules-${Date.now()}`;
...
const diffs = users.map((u) => {
  ...
  // Check if needs update for target modules only
  const targetState = {};
  for (const mod of TARGET_MODULES) {
    targetState[mod] = true;
  }
...
console.log(`   módulos alvo:            ${TARGET_MODULES.join(', ')}`);
...
if (!apply) {
  ...
  console.log(
    `   Pra aplicar: node scripts/provision-modules-claims.mjs${moduleArg ? ` --module ${moduleArg}` : ''} --apply --reason "…"\n`,
  );
  process.exit(0);
}
...
await db.collection('auditLogs').add({
  action: apply ? 'PROVISION_MODULES_CLAIM_APPLY_LOCAL' : 'PROVISION_MODULES_CLAIM_DRY_RUN',
