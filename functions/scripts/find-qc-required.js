  ...
  try {
    ...
    if (withQCRequired.length > 0) {
      ...
      withQCRequired.forEach(doc => {
        ...
        console.log(`   dataAbertura: ${data.dataAbertura ? new Date(data.dataAbertura.toDate()).toLocaleString('pt-BR') : 'null'}`);
      ...
      insumosSnapshot.docs.forEach(doc => {
        const data = doc.data();
        console.log(`   ${data.lote} — qcValidationRequired: ${data.qcValidationRequired || 'undefined'}`);
      });
    }
