...
lotsSnap.forEach(doc => {
  const d = doc.data();
  if (d.lotNumber === 'HHI-1375' || d.lotNumber === 'HHI-1372' || d.lotNumber === 'HHI-1376' || d.lotNumber === 'HHI-1377') {
...
for (const lot of targets) {
  ...
  const runsSnap = await db.collection('labs').doc(labId).collection('lots').doc(lot.id).collection('runs').get();
  ...
  runsSnap.forEach(r => {
    ...
    const created = rd.createdAt?.toDate ? rd.createdAt.toDate().toISOString().slice(0,16) : rd.createdAt;
    console.log(`    run ${r.id.slice(0,8)} | date=${date} | created=${created} | analytes=${Object.keys(rd.results || rd.values || {}).length} | hidden=${rd.hidden ?? rd.manualHidden ?? false}`);
    // print first analyte
