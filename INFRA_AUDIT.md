...
Embora as funções exijam login do usuário, a validação desse login ocorre _dentro_ do código da função (camada de aplicação). A camada de rede (Google Cloud) não conhece os usuários do Firebase, por isso deve "deixar passar" a requisição para que a função valide o token internamente.
...
4. No campo **Autenticação**, selecionar **"Permitir invocações não autenticadas"**.
   - _Nota: Se esta opção estiver cinza/bloqueada, você deve primeiro desabilitar a política de organização "Ativo: Restringir Compartilhamento de Domínio" (Domain Restricted Sharing) nas configurações de IAM & Admin > Organization Policies._
