...
test('debug login flow', async ({ page }) => {
  ...
  if (emailVisible) {
    ...
    console.log(
      'Sair button visible:',
      await sairBtn.isVisible({ timeout: 2000 }).catch(() => false),
    );

    // Check for lab selector
    const labText = page.locator('text=Selecione').first();
    console.log(
      'Lab selector text visible:',
      await labText.isVisible({ timeout: 2000 }).catch(() => false),
    );

    // Check for Coagulação v2
    const coag = page.locator('text=Coagulação').first();
    console.log(
      'Coagulação text visible:',
      await coag.isVisible({ timeout: 2000 }).catch(() => false),
    );
  }
