...
async function abrirNovaCorridaCoag(page: import('@playwright/test').Page) {
  ...
  if (await clotBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
    ...
    const eqSelect = page
      ...
      .filter({ hasText: /Clotimer|CLOT/i })
      .first();
...
test.describe('F-QuickSetup-01 — Lote fechado aparece e é aberto automaticamente', () => {
  test('Criar lote fechado → QuickSetupInline detecta, exibe label "(fechado — será aberto)", abre e vincula', async ({
    loggedInPage: page,
  }) => {
    ...
    const quickSetup = dialog.locator('text=Setup rápido — vincular insumos ao equipamento');
    ...
    const reagenteSelect = dialog
      .locator('select')
      .filter({ hasText: /Selecione/ })
      .first();
    // Verifica que a option com "(fechado — será aberto)" existe
    const optionFechado = dialog.locator(`option:has-text("${LOTE_FECHADO_REAGENTE}")`).first();
    ...
    await expect(dialog.locator('button:has-text("Salvando…")'))
    ...
    const quickSetupDepois = dialog.locator('text=Setup rápido — vincular insumos ao equipamento');
    // Pode ter sumido ou continuar visível se outros slots ainda faltam —
    // apenas verificamos que a conferência progrediu
    console.log(`  QuickSetupInline ainda visível: ${await quickSetupDepois.isVisible()}`);
...
test.describe('F-QuickSetup-02 — Lote ativo continua funcionando (regressão)', () => {
  test('Criar lote ativo → QuickSetupInline vincula normalmente, SEM label "(fechado — será aberto)"', async ({
    loggedInPage: page,
  }) => {
    test.setTimeout(300_000);
    ...
    if (await quickSetup.isVisible({ timeout: 8_000 }).catch(() => false)) {
      const optionAtivo = dialog.locator(`option:has-text("${LOTE_ATIVO_REAGENTE}")`).first();
      ...
      console.log('  QuickSetupInline não apareceu (reagente ativo já configurado no setup) — OK.');
    }
...
test.describe('F-QuickSetup-03 — Cenário misto: reagente fechado + controle ativo', () => {
  test('Abre só o lote que precisa (fechado), não retoca lote ativo', async ({
    loggedInPage: page,
  }) => {
    test.setTimeout(300_000);
    ...
    const optFechadoMix = dialog.locator(`option:has-text("SMOKE-MIX-RG-${ts}")`).first();
    ...
    const optAtivoMix = dialog.locator(`option:has-text("SMOKE-MIX-CT-${ts}")`).first();
...
test.describe('F-QuickSetup-04 — Sem lotes disponíveis: mensagem clara', () => {
  test('Quando não há lotes compatíveis, QuickSetupInline NÃO aparece e exibe mensagem orientadora', async ({
    loggedInPage: page,
  }) => {
    ...
    const quickSetup = dialog.locator('text=Setup rápido — vincular insumos ao equipamento');
    const slotFaltando = dialog.locator('text=Há insumo obrigatório não configurado');
    ...
    if (hasSlotFaltando && !hasQuickSetup) {
      ...
      const msgOrientadora = dialog
        .locator('text=/configure no Setup|Configure no Setup|cadastrar/i')
        .first();
      ...
      console.log('  Setup completo — nenhum slot faltando (conferência mostra OK para todos).');
    }
...
test.describe('F-QuickSetup-05 — Regressão: Uroanálise', () => {
  test('Lotes fechados de tiras de uroanálise aparecem com "(fechado — será aberto)"', async ({
    loggedInPage: page,
  }) => {
    test.setTimeout(300_000);
    ...
    const quickSetupUro = dialog.locator('text=Setup rápido — vincular insumos ao equipamento');
    ...
    if (await quickSetupUro.isVisible({ timeout: 8_000 }).catch(() => false)) {
      ...
      const optTiraFechada = dialog.locator(`option:has-text("${LOTE_FECHADO_TIRA}")`).first();
