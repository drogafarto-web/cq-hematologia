...
### `AmostraStatus`
```typescript
type AmostraStatus = 'coletada' | 'recebida' | 'em_processamento' | 'concluida' | 'rejeitada' | 'descartada';
```

### `MotivoRejeicao`
```typescript
type MotivoRejeicao = 'volume_insuficiente' | 'hemolise' | 'contaminacao' | 'identificacao_incorreta' | 'outros';
```
